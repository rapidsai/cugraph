...
from pylibcugraph import homogeneous_uniform_neighbor_sample as pylibcugraph_homogeneous_uniform_neighbor_sample
...
def _call_plc_homogeneous_uniform_neighbor_sample(
    sID,
    mg_graph_x,
    st_x,
    ...
    cupy_array_dict = pylibcugraph_homogeneous_uniform_neighbor_sample(
...
def _mg_call_plc_homogeneous_uniform_neighbor_sample(
    client,
    session_id,
    input_graph,
    ddf,
    ...
    result = [
        client.submit(
            _call_plc_homogeneous_uniform_neighbor_sample,
            session_id,
...
def homogeneous_uniform_neighbor_sample(
    input_graph: Graph,
    start_list: Sequence,
    ...
    *,
    ...
    result : dask_cudf.DataFrame or Tuple[dask_cudf.DataFrame, dask_cudf.DataFrame]
        ...
        If return_offsets=False:
                ...
                df['edge_weight']: dask_cudf.Series # if provided 
                    Contains the edge weights from the sampling result
                df['edge_id']: dask_cudf.Series # if provided 
                    Contains the edge ids from the sampling result
                df['edge_type']: dask_cudf.Series # if provided 
                ...
                If renumber=True:
                    ...
                    renumber_df['renumber_map']: dask_cudf.Series
                        Contains the renumber maps for each batch
                    renumber_df['batch_id']: dask_cudf.Series
    ...
    if renumber and not keep_batches_together:
        raise ValueError(
            "mg homogeneous_uniform_neighbor_sample requires that keep_batches_together=True "
    ...
    if _multiple_clients:
        ...
        if lock.acquire(timeout=100):
            try:
                ddf = _mg_call_plc_homogeneous_uniform_neighbor_sample(**sample_call_kwargs)
            finally:
                lock.release()
        else:
            raise RuntimeError(
