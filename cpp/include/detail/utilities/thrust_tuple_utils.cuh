...
struct compute_thrust_tuple_element_sizes_impl {
  void compute(
    std::array<size_t, thrust::tuple_size<TupleType>::value>& arr) const {
...
__device__ constexpr auto remove_first_thrust_tuple_element_impl(
...
struct is_thrust_tuple_of_arithmetic<
  TupleType, std::enable_if_t<is_thrust_tuple<TupleType>::value>
> {
  static constexpr bool value =
    is_thrust_tuple_of_arithemetic_impl<
      TupleType,
...
struct compute_thrust_tuple_element_sizes {
  auto operator()() const {
    ...
    compute_thrust_tuple_element_sizes_impl<
      TupleType, static_cast<size_t>(0), tuple_size
    >().compute(ret);
    return ret;
  }
};
...
struct remove_first_thrust_tuple_element {
  __device__ constexpr auto operator()(TupleType const& tuple) const {
    size_t constexpr tuple_size = thrust::tuple_size<TupleType>::value;
    return remove_first_thrust_tuple_element_impl(
...
struct block_reduce_thrust_tuple {
  __device__ TupleType operator()(TupleType const& tuple) const {
    ...
    block_reduce_thrust_tuple_impl<
      TupleType, BlockSize, static_cast<size_t>(0), tuple_size
    >().compute(ret);
    return ret;
  }
};
