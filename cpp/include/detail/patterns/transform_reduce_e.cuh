...
template <typename T>
__device__
std::enable_if_t<cugraph::experimental::detail::is_thrust_tuple<T>::value, T>
plus_edge_op_result(T const& lhs, T const& rhs) {
...
struct block_reduce_edge_op_result {
  ...
  __device__
  std::enable_if_t<std::is_arithmetic<T>::value, T>
  compute(T const& edge_op_result) {
  ...
  std::enable_if_t<cugraph::experimental::detail::is_thrust_tuple<T>::value, T>
  compute(T const& edge_op_result) {
    return cugraph::experimental::detail::block_reduce_thrust_tuple<
      T, BlockSize>()(edge_op_result);
  }
};
...
template <typename GraphType,
          ...
          typename EdgeOp>
__global__
void for_all_row_for_all_nbr_col_low_out_degree(
    GraphType graph_device_view,
    RowIterator row_first, RowIterator row_last,
    AdjMatrixRowValueInputIterator adj_matrix_row_value_input_first,
    AdjMatrixColValueInputIterator adj_matrix_col_value_input_first,
    BlockResultIterator block_result_first, EdgeOp e_op) {
  ...
  static_assert(GraphType::is_csr_type || GraphType::is_csc_type);
  ...
  e_op_result_sum =
    block_reduce_edge_op_result<
      e_op_result_t, transform_reduce_e_for_all_low_out_degree_block_size
    >().compute(e_op_result_sum);
  if (threadIdx.x == 0) {
    *(block_result_first + blockIdx.x) = e_op_result_sum;
  }
}
...
T transform_reduce_e(
  ...
  for_all_row_for_all_nbr_col_low_out_degree<<<
    update_grid.num_blocks, update_grid.block_size, 0, handle.get_default_stream()
  >>>(
    graph,
    graph.this_partition_adj_matrix_row_begin(), graph.this_partition_adj_matrix_row_end(),
    adj_matrix_row_value_input_first, adj_matrix_col_value_input_first,
    block_results.data(), e_op);
