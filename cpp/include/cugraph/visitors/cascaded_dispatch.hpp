...
constexpr pair_uniques_t graph_dispatcher(GTypes graph_type, erased_pack_t& ep)
{
  switch (graph_type) {
    case GTypes::GRAPH_T: {
      using graph_t = typename GMapType<vertex_t, edge_t, weight_t, mg, GTypes::GRAPH_T>::type;
      graph_factory_t<graph_t> factory;

      pair_uniques_t p_uniques =
        std::make_pair(factory.make_graph(ep),
                       std::make_unique<dependent_factory_t<vertex_t, edge_t, weight_t, mg>>());
...
constexpr decltype(auto) multi_gpu_dispatcher(bool multi_gpu, GTypes graph_type, erased_pack_t& ep)
{
  switch (multi_gpu) {
    case true: {
      return graph_dispatcher<vertex_t, edge_t, weight_t, true>(graph_type, ep);
    } break;
    case false: {
      return graph_dispatcher<vertex_t, edge_t, weight_t, false>(graph_type, ep);
    }
  }
}
                                           ...
                                           erased_pack_t& ep)
{
  switch (weight_type) {
    case DTypes::INT32: {
      ...
      return multi_gpu_dispatcher<vertex_t, edge_t, weight_t>(multi_gpu, graph_type, ep);
    } break;
    case DTypes::INT64: {
...
//
template <typename vertex_t>
constexpr decltype(auto) edge_dispatcher(
  DTypes edge_type, DTypes weight_type, bool multi_gpu, GTypes graph_type, erased_pack_t& ep)
{
  switch (edge_type) {
    case DTypes::INT32: {
      using edge_t = typename DMapType<DTypes::INT32>::type;
      return weight_dispatcher<vertex_t, edge_t>(weight_type, multi_gpu, graph_type, ep);
    } break;
    case DTypes::INT64: {
                                        ...
                                        erased_pack_t& ep)
{
  switch (vertex_type) {
    case DTypes::INT32: {
      ...
      return edge_dispatcher<vertex_t>(edge_type, weight_type, multi_gpu, graph_type, ep);
    } break;
    case DTypes::INT64: {
