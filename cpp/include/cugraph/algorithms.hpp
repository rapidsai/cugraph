 ...
 * @param degree_type Dictate whether to compute the K-core decomposition based on in-degrees, out-degrees, or in-degrees + out_degrees.
 * @param k_first Find K-cores from K = k_first. Any vertices that do not belong to k_first-core will have core numbers of 0.
 * @param k_last Find K-cores to K = k_last. Any vertices that belong to (k_last + 1) core will have core numbers of k_last.
 * @param do_expensive_check A flag to run expensive checks for input arguments (if set to `true`).
 */
template <typename vertex_t, typename edge_t, typename weight_t, bool multi_gpu>
void core_number(
