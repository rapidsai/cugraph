 ...
 * @tparam vertex_t Type of vertex identifiers. Needs to be an integral type.
 * @tparam edge_t Type of edge identifiers. Needs to be an integral type.
 * @tparam property1_t Type of first optional edge property
 * @tparam property2_t Type of second optional edge property
 * @tparam property3_t Type of third optional edge property
 * @tparam store_transposed Flag indicating whether to use sources (if false) or destinations (if
 * true) as major indices in storing edges using a 2D sparse matrix. transposed.
 * @tparam multi_gpu Flag indicating whether template instantiation should target single-GPU (false)
 * or multi-GPU (true).
 ...
 * function to work (edges should be pre-shuffled).
 * @param edgelist_dsts Vector of edge destination vertex IDs.
 * @param edgelist_property1 Vector of property1 values for edges
 * @param edgelist_property2 Vector of property2 values for edges
 * @param edgelist_property3 Vector of property3 values for edges
...
template <typename vertex_t,
          typename edge_t,
          typename property1_t,
          typename property2_t,
          typename property3_t,
          bool store_transposed,
          bool multi_gpu>
std::tuple<
  graph_t<vertex_t, edge_t, store_transposed, multi_gpu>,
  std::optional<
    edge_property_t<graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu>, property1_t>>,
  std::optional<
    edge_property_t<graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu>, property2_t>>,
  std::optional<
    edge_property_t<graph_view_t<vertex_t, edge_t, store_transposed, multi_gpu>, property3_t>>,
  std::optional<rmm::device_uvector<vertex_t>>>
create_graph_from_edgelist(raft::handle_t const& handle,
                           std::optional<rmm::device_uvector<vertex_t>>&& vertices,
                           rmm::device_uvector<vertex_t>&& edgelist_srcs,
                           rmm::device_uvector<vertex_t>&& edgelist_dsts,
                           std::optional<rmm::device_uvector<property1_t>>&& edgelist_property1,
                           std::optional<rmm::device_uvector<property2_t>>&& edgelist_property2,
                           std::optional<rmm::device_uvector<property3_t>>&& edgelist_property3,
