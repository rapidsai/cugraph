  ...
  T* value)
{
  ...
  auto row_offset   = storage_transposed ? minor_offset : major_offset;
  auto col_offset   = storage_transposed ? major_offset : minor_offset;

  *key = adj_matrix_row_col_key_input.get(
    ((storage_transposed != adj_matrix_row_key) ? major_offset : minor_offset));
  *value = evaluate_edge_op<vertex_t,
                            weight_t,
                            vertex_t,
  ...
  T* values)
{
  ...
  while (idx < static_cast<size_t>(dcs_nzd_vertex_count)) {
    ...
    for (edge_t i = 0; i < local_degree; ++i) {
      update_buffer_element<adj_matrix_row_key, storage_transposed>(
        matrix_partition,
        major,
        indices[i],
...
void traverse_matrix_partition_edges(
  ...
  AdjMatrixRowColKeyInputWrapper matrix_partition_row_col_key_input,
  vertex_t* keys,
  T* values,
  EdgeOp e_op,
  std::optional<std::vector<vertex_t>> const& segment_offsets)
{
  if (segment_offsets) {
    ...
    if ((*segment_offsets)[1] > 0) {
      ...
      detail::for_all_major_for_all_nbr_high_degree<adj_matrix_row_key, storage_transposed>
    ...
    if ((*segment_offsets)[2] - (*segment_offsets)[1] > 0) {
      raft::grid_1d_warp_t update_grid(
        ...
        detail::transform_reduce_by_adj_matrix_row_col_key_e_for_all_block_size,
        handle.get_device_properties().maxGridSize[0]);
      detail::for_all_major_for_all_nbr_mid_degree<adj_matrix_row_key, storage_transposed>
        <<<update_grid.num_blocks, update_grid.block_size, 0, handle.get_stream()>>>(
          matrix_partition,
          matrix_partition.get_major_first() + (*segment_offsets)[1],
    ...
    if ((*segment_offsets)[3] - (*segment_offsets)[2] > 0) {
      raft::grid_1d_thread_t update_grid(
        ...
        detail::transform_reduce_by_adj_matrix_row_col_key_e_for_all_block_size,
        handle.get_device_properties().maxGridSize[0]);
      detail::for_all_major_for_all_nbr_low_degree<adj_matrix_row_key, storage_transposed>
        <<<update_grid.num_blocks, update_grid.block_size, 0, handle.get_stream()>>>(
          matrix_partition,
          matrix_partition.get_major_first() + (*segment_offsets)[2],
    ...
    if (matrix_partition.get_dcs_nzd_vertex_count() &&
      ...
      raft::grid_1d_thread_t update_grid(
        *(matrix_partition.get_dcs_nzd_vertex_count()),
        detail::transform_reduce_by_adj_matrix_row_col_key_e_for_all_block_size,
        handle.get_device_properties().maxGridSize[0]);
      detail::for_all_major_for_all_nbr_hypersparse<adj_matrix_row_key, storage_transposed>
        <<<update_grid.num_blocks, update_grid.block_size, 0, handle.get_stream()>>>(
          matrix_partition,
          matrix_partition.get_major_first() + (*segment_offsets)[3],
          matrix_partition_row_value_input,
  ...
  T init)
{
  ...
  for (size_t i = 0; i < graph_view.get_number_of_local_adj_matrix_partitions(); ++i) {
    ...
    if (graph_view.get_vertex_partition_size(comm_root_rank) > 0) {
      ...
      if ((adj_matrix_row_key && !graph_view.storage_transposed()) ||
          (!adj_matrix_row_key && graph_view.storage_transposed())) {
      ...
      if (graph_view.storage_transposed()) {
        traverse_matrix_partition_edges<adj_matrix_row_key, true>(
          handle,
          matrix_partition,
          matrix_partition_row_value_input,
          matrix_partition_col_value_input,
        ...
        traverse_matrix_partition_edges<adj_matrix_row_key, false>(
          handle,
          matrix_partition,
          matrix_partition_row_value_input,
          matrix_partition_col_value_input,
