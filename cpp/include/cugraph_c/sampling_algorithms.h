 ...
 * id of src_bias[i] and dst_bias[i].  If @p vertices is not specified then i is the vertex id if
 * src_bias[i] and dst_bias[i]
 * @param [in]     src_bias                Bias for selecting source vertices.  If NULL, do uniform
 *                                         sampling, if provided probability of vertex i will be
 *                                         src_bias[i] / (sum of all source biases)
 * @param [in]     dst_bias                Bias for selecting destination vertices.  If NULL, do
 ...
 * @param [in]     remove_false_negatives  If true, remove sampled edges that actually exist in the
 * graph
