...
public:
  ...
  Leiden(graph_type const &graph, cudaStream_t stream):
    Louvain<graph_type>(graph, stream),
    constraint_v_(graph.number_of_vertices) {
  }

  weight_t update_clustering_constrained(weight_t total_edge_weight, weight_t resolution,
    ...
    weight_t new_Q = this->modularity(total_edge_weight, resolution, graph, this->cluster_v_.data().get());
    ...
    while (new_Q > (cur_Q + 0.0001)) {
      ...
      this->compute_delta_modularity(total_edge_weight, resolution, graph,
                                     cluster_hash_v,
                                     old_cluster_sum_v,
                                     delta_Q_v);
  ...
  std::pair<int, weight_t> compute(vertex_t *d_cluster_vec,
                                   int max_level,
                                   weight_t resolution) {
    int num_level{0};

    weight_t total_edge_weight =
      thrust::reduce(rmm::exec_policy(this->stream_)->on(this->stream_), this->weights_v_.begin(), this->weights_v_.end());
    ...
    //
    thrust::sequence(rmm::exec_policy(this->stream_)->on(this->stream_), this->cluster_v_.begin(), this->cluster_v_.end());
    thrust::copy(
      rmm::exec_policy(this->stream_)->on(this->stream_), this->cluster_v_.begin(), this->cluster_v_.end(), d_cluster_vec);
    ...
    while (num_level < max_level) {
      ...
      thrust::copy(
        rmm::exec_policy(this->stream_)->on(this->stream_), this->cluster_v_.begin(), this->cluster_v_.end(), constraint_v_.begin());

      new_Q = update_clustering_constrained(total_edge_weight,
                                            resolution,
                                            current_graph);
