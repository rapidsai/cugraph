...
template<>
__global__ void
generate_cluster_inv_ptr<int64_t*, int64_t>(const int n_vertex, int64_t* cluster_iter, int64_t* cluster_inv_ptr) {
  int tid = blockDim.x * blockIdx.x + threadIdx.x;
  int64_t ci;
  if (tid < n_vertex) {
    ci = *(cluster_iter + tid);
    unsigned long long int *cast = (unsigned long long int *)cluster_inv_ptr;
    atomicAdd(cast + ci, 1);
  }
}
...
void generate_cluster_inv(const IdxType n_vertex,
  ...
  int nthreads = min(n_vertex, (IdxType)CUDA_MAX_KERNEL_THREADS);
  int nblocks = min((n_vertex + nthreads - 1)/nthreads,(IdxType)CUDA_MAX_BLOCKS);
...
void generate_supervertices_graph(const int n_vertex,
  ...
  rmm::device_vector<IndexType> I_out(NNZ, 0);
  rmm::device_vector<IndexType> J_out(NNZ, 0);
  rmm::device_vector<ValueType> V_out(NNZ, 0);
  ...
  thrust::copy(I_out.begin(), I_out.end(), new_row_indices.begin());
  thrust::copy(J_out.begin(), J_out.end(), *new_csr_ind);
  thrust::copy(V_out.begin(), V_out.end(), *new_csr_val);
