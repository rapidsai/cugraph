...
class Louvain {
 public:
  ...
  {
    thrust::copy(rmm::exec_policy(handle_.get_stream()),
                 graph.offsets,
                      ...
                      vertex_t const* d_cluster)
  {
    ...
    thrust::fill(
      rmm::exec_policy(handle_.get_stream()), inc.begin(), inc.end(), weight_t{0.0});
    thrust::fill(
      rmm::exec_policy(handle_.get_stream()), deg.begin(), deg.end(), weight_t{0.0});
    ...
    //
    thrust::for_each(rmm::exec_policy(handle_.get_stream()),
  ...
  virtual weight_t operator()(size_t max_level, weight_t resolution)
  {
    weight_t total_edge_weight = thrust::reduce(
      rmm::exec_policy(handle_.get_stream()), weights_v_.begin(), weights_v_.end());
 ...
 protected:
  ...
  virtual void initialize_dendrogram_level(vertex_t num_vertices)
  {
    ...
    thrust::sequence(rmm::exec_policy(handle_.get_stream()),
 ...
 public:
                    ...
                    bool up_down)
  {
    ...
    auto cluster_reduce_end =
      thrust::reduce_by_key(rmm::exec_policy(handle_.get_stream()),
  ...
  void generate_superverticies_graph(graph_t& graph, vertex_t num_clusters)
  {
    ...
    auto new_end = thrust::reduce_by_key(rmm::exec_policy(handle_.get_stream()),
                                         start,
