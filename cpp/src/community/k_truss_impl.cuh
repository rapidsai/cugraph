    ...
    [num_invalid_edges = invalid_edgelist_dsts.size(),
     ...
     invalid_last = thrust::make_zip_iterator(invalid_edgelist_srcs.end(),
      ...
      auto potential_or_incoming_edge =
        thrust::make_tuple(thrust::get<0>(potential_edge), thrust::get<1>(potential_edge));
      if constexpr (is_p_q_edge) {
        potential_or_incoming_edge =
          thrust::make_tuple(thrust::get<1>(potential_edge), thrust::get<0>(potential_edge));
      };

      auto itr =
        thrust::lower_bound(thrust::seq, invalid_first, invalid_last, potential_or_incoming_edge);
...
struct extract_weak_edges {
  ...
  {
    return count < k - 2
             ? thrust::optional<thrust::tuple<vertex_t, vertex_t, edge_t>>{thrust::make_tuple(
                 src, dst, count)}
             : thrust::nullopt;
  }
};
...
struct extract_edges_to_q_r {
    ...
    auto src, auto dst, thrust::nullopt_t, thrust::nullopt_t, thrust::nullopt_t) const
  {
    ...
    auto itr_src = thrust::find(thrust::seq, vertex_q_r.begin(), vertex_q_r.end(), src);

    // FIXME: Replace by lowerbound after validation
    auto itr_dst = thrust::find(thrust::seq, vertex_q_r.begin(), vertex_q_r.end(), dst);
...
struct generate_p_q_q_r {
  ...
  __device__ thrust::tuple<vertex_t, vertex_t> operator()(edge_t i) const
  {
    if constexpr (generate_p_q) {
      return thrust::make_tuple(thrust::get<0>(*(invalid_edge + invalid_edge_idx[i])),
                                q_closing[i]);
    } else {
      return thrust::make_tuple(q_closing[i],
                                thrust::get<1>(*(invalid_edge + invalid_edge_idx[i])));
    }
  }
};
  ...
  raft::device_span<vertex_t> vertex_pair_buffer_dst)
{
  ...
  auto vertex_pair_buffer_begin =
    thrust::make_zip_iterator(vertex_pair_buffer_src.begin(), vertex_pair_buffer_dst.begin());
  ...
  thrust::fill(
    handle.get_thrust_policy(), decrease_count_tmp.begin(), decrease_count_tmp.end(), size_t{1});
  ...
  cugraph::transform_e(
    ...
    [vertex_pair_buffer_begin = get_dataframe_buffer_begin(vertex_pair_buffer_unique),
     ...
     decrease_count           = decrease_count.data()] __device__(auto src,
      ...
      auto itr_pair =
        thrust::lower_bound(thrust::seq, vertex_pair_buffer_begin, vertex_pair_buffer_end, e);
...
template <typename vertex_t,
          typename edge_t,
          typename weight_t,
          typename optional_graph_view_t,
          bool multi_gpu,
          bool is_q_r_edge,
          bool is_p_q_edge>
void find_unroll_p_q_q_r_edges(
  raft::handle_t const& handle,
  graph_view_t<vertex_t, edge_t, false, multi_gpu>& cur_graph_view,
  ...
  for (size_t i = 0; i < num_chunks; ++i) {
    ...
    if constexpr (!is_p_q_edge) {
      if constexpr (multi_gpu) {
        ...
        auto global_weak_edgelist_srcs = cugraph::detail::device_allgatherv(
          handle, comm, raft::device_span<vertex_t const>(weak_edgelist_srcs));

        auto global_weak_edgelist_dsts = cugraph::detail::device_allgatherv(
          handle, comm, raft::device_span<vertex_t const>(weak_edgelist_dsts));

        weak_edgelist_srcs = raft::device_span<vertex_t>(global_weak_edgelist_srcs.data(),
                                                         global_weak_edgelist_srcs.size());
        weak_edgelist_dsts = raft::device_span<vertex_t>(global_weak_edgelist_dsts.data(),
                                                         global_weak_edgelist_dsts.size());

        // Sort the weak edges if they are not already
        auto invalid_edgelist =
          thrust::make_zip_iterator(weak_edgelist_srcs.begin(), weak_edgelist_dsts.begin());
        thrust::sort(handle.get_thrust_policy(),
                     invalid_edge_first,
    ...
    if constexpr (is_p_q_edge) {
      ...
      resize_dataframe_buffer(
        vertex_pair_buffer_p_r_edge_p_q, num_edges_not_overcomp, handle.get_stream());
      resize_dataframe_buffer(
        vertex_pair_buffer_q_r_edge_p_q, num_edges_not_overcomp, handle.get_stream());
      ...
      thrust::tabulate(
        ...
        [vertex_pair_buffer_p_r_edge_p_q =
         ...
         vertex_pair_buffer_q_r_edge_p_q =
           get_dataframe_buffer_begin(vertex_pair_buffer_q_r_edge_p_q)] __device__(auto i) {
          return thrust::make_tuple(thrust::get<0>(vertex_pair_buffer_p_r_edge_p_q[i]),
                                    thrust::get<0>(vertex_pair_buffer_q_r_edge_p_q[i]));
        });
    }
    ...
    if constexpr (multi_gpu) {
      if constexpr (is_q_r_edge) {
        auto vertex_partition_range_lasts = std::make_optional<std::vector<vertex_t>>(
          (*graph_q_r).view().vertex_partition_range_lasts());
      ...
      if constexpr (is_p_q_edge) {
        update_count<vertex_t, edge_t, weight_t, multi_gpu>(
          ...
          e_property_triangle_count,
          tmp_edge_mask,
          raft::device_span<vertex_t>(std::get<0>(vertex_pair_buffer_p_r_edge_p_q).data(),
                                      std::get<0>(vertex_pair_buffer_p_r_edge_p_q).size()),
          raft::device_span<vertex_t>(std::get<1>(vertex_pair_buffer_p_r_edge_p_q).data(),
                                      std::get<1>(vertex_pair_buffer_p_r_edge_p_q).size()));
      } else {
        update_count<vertex_t, edge_t, weight_t, multi_gpu>(
          handle,
        ...
        bool do_expensive_check)
{
...

  {
    ...
    cugraph::edge_property_t<graph_view_t<vertex_t, edge_t, false, multi_gpu>, bool> tmp_edge_mask(
    ...
    while (true) {
      ...
      auto [weak_edgelist_srcs, weak_edgelist_dsts, triangle_count] =
        extract_transform_e(handle,
                            cur_graph_view,
                            edge_src_dummy_property_t{}.view(),
                            edge_dst_dummy_property_t{}.view(),
      ...
      if (num_invalid_edges == 0 && done == 0) { break; }
      ...
      edge_property_t<decltype(cur_graph_view), edge_t> modified_triangle_count(handle,
                                                                                cur_graph_view);

      std::optional<graph_t<vertex_t, edge_t, false, multi_gpu>> dummy_graph{std::nullopt};
      find_unroll_p_q_q_r_edges<vertex_t,
                                edge_t,
                                weight_t,
                                decltype(dummy_graph),
                                multi_gpu,
                                false,
                                true>(
        handle,
      ...
      auto invalid_unique_v_end =
        thrust::unique(handle.get_thrust_policy(), vertex_q_r.begin(), vertex_q_r.end());

      vertex_q_r.resize(thrust::distance(vertex_q_r.begin(), invalid_unique_v_end),
                        handle.get_stream());

      auto invalid_edgelist =
        thrust::make_zip_iterator(weak_edgelist_srcs.begin(), weak_edgelist_dsts.begin());

      auto [srcs_to_q_r, dsts_to_q_r] =
        extract_transform_e(handle,
                            cur_graph_view,
                            cugraph::edge_src_dummy_property_t{}.view(),
                            cugraph::edge_dst_dummy_property_t{}.view(),
                            ...
                            extract_edges_to_q_r<vertex_t>{raft::device_span<vertex_t const>(
                              vertex_q_r.data(), vertex_q_r.size())});
      ...
      // FIXME: Need to mask (p, q) and (q, r) edges before unrolling (p, r) edges to avoid
      // overcompensating
      auto [q_closing, idx_closing] = cugraph::extract_transform_v_frontier_outgoing_e(
        handle,
        cur_graph_view,
        vertex_frontier.bucket(0),
        cugraph::edge_src_dummy_property_t{}.view(),
      ...
      thrust::tabulate(handle.get_thrust_policy(),
                       ...
                       generate_p_r<vertex_t, edge_t, decltype(invalid_edgelist)>{
                         invalid_edgelist,
                         raft::device_span<edge_t const>(idx_closing.data(), idx_closing.size())});
      ...
      thrust::tabulate(handle.get_thrust_policy(),
                       ...
                       generate_p_q_q_r<vertex_t, edge_t, decltype(invalid_edgelist), true>{
                         invalid_edgelist,
                         raft::device_span<vertex_t const>(q_closing.data(), q_closing.size()),
                         raft::device_span<edge_t const>(idx_closing.data(), idx_closing.size())});
      ...
      if constexpr (multi_gpu) {
        ...
        // Sort the weak edges if they are not already
        auto invalid_edgelist = thrust::make_zip_iterator(global_weak_edgelist_srcs.begin(),
                                                          global_weak_edgelist_dsts.begin());
        thrust::sort(handle.get_thrust_policy(),
                     invalid_edge_first,
                     invalid_edge_first + weak_edgelist_srcs.size());
        ...
        resize_dataframe_buffer(
          vertex_pair_buffer_p_q_for_p_r, num_edges_not_overcomp_p_q, handle.get_stream());
        resize_dataframe_buffer(
          vertex_pair_buffer_q_r_for_p_r, num_edges_not_overcomp_p_q, handle.get_stream());
        ...
        resize_dataframe_buffer(
          vertex_pair_buffer_p_q_for_p_r, num_edges_not_overcomp_q_r, handle.get_stream());
        resize_dataframe_buffer(
          vertex_pair_buffer_q_r_for_p_r, num_edges_not_overcomp_q_r, handle.get_stream());
        ...
        auto vertex_pair_buffer_p_r = allocate_dataframe_buffer<thrust::tuple<vertex_t, vertex_t>>(
          pair_q_r_srcs.size(), handle.get_stream());
        ...
        thrust::tabulate(
          ...
          [vertex_pair_buffer_p_q_for_p_r =
             get_dataframe_buffer_begin(vertex_pair_buffer_p_q_for_p_r),
           vertex_pair_buffer_q_r_for_p_r =
             get_dataframe_buffer_begin(vertex_pair_buffer_q_r_for_p_r)] __device__(auto i) {
            return thrust::make_tuple(thrust::get<0>(vertex_pair_buffer_p_q_for_p_r[i]),
                                      thrust::get<1>(vertex_pair_buffer_q_r_for_p_r[i]));
          });
        ...
        update_count<vertex_t, edge_t, weight_t, multi_gpu>(
          ...
          e_property_triangle_count,
          tmp_edge_mask,
          raft::device_span<vertex_t>(std::get<0>(vertex_pair_buffer_p_q_for_p_r).data(),
                                      std::get<0>(vertex_pair_buffer_p_q_for_p_r).size()),
          raft::device_span<vertex_t>(std::get<1>(vertex_pair_buffer_p_q_for_p_r).data(),
                                      std::get<1>(vertex_pair_buffer_p_q_for_p_r).size()));
        ...
        update_count<vertex_t, edge_t, weight_t, multi_gpu>(
          ...
          e_property_triangle_count,
          tmp_edge_mask,
          raft::device_span<vertex_t>(std::get<0>(vertex_pair_buffer_q_r_for_p_r).data(),
                                      std::get<0>(vertex_pair_buffer_q_r_for_p_r).size()),
          raft::device_span<vertex_t>(std::get<1>(vertex_pair_buffer_q_r_for_p_r).data(),
                                      std::get<1>(vertex_pair_buffer_q_r_for_p_r).size()));
      }
      ...
      if (edge_weight_view) {
        auto [edgelist_srcs, edgelist_dsts, edgelist_count] = extract_transform_e(
          handle,
          cur_graph_view,
          cugraph::edge_src_dummy_property_t{}.view(),
          cugraph::edge_dst_dummy_property_t{}.view(),
