...
struct generate_p_r {
  ...
  __device__ thrust::tuple<vertex_t, vertex_t> operator()(edge_t i) const
  {
    ...
    auto pair = thrust::make_tuple(thrust::get<0>(*(edge_first + idx)), intersection_indices[i]);
...
struct generate_q_r {
  ...
  __device__ thrust::tuple<vertex_t, vertex_t> operator()(edge_t i) const
  {
    auto itr = thrust::upper_bound(
      thrust::seq, intersection_offsets.begin() + 1, intersection_offsets.end(), i);
    auto idx  = thrust::distance(intersection_offsets.begin() + 1, itr);
    auto pair = thrust::make_tuple(thrust::get<1>(*(edge_first + idx)), intersection_indices[i]);
             ...
             bool do_expensive_check)
{
...

  {
    ...
    while (true) {
      ...
      num_invalid_edges =
        static_cast<size_t>(thrust::distance(edge_triangle_count_pair_first, invalid_edge_last_));
    ...
    if (num_invalid_edges == edgelist_srcs.size()) {
      ...
      std::optional<graph_view_t<vertex_t, edge_t, false, multi_gpu>> empty_graph_view{
        std::nullopt};
