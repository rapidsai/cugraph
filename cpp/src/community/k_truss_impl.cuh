...

{
  ...
  auto edge_exists = graph_view.has_edge(
                  handle,
    ...
    while (true) {
        ...
        if constexpr (multi_gpu) {
          ...
          edge_exists = cur_graph_view.has_edge(
                          ...
                          raft::device_span<vertex_t const>(
                            std::get<2>(triangles_from_weak_edges).data(), intersection_indices.size()),
                          raft::device_span<vertex_t const>(
                            std::get<3>(triangles_from_weak_edges).data(), intersection_indices.size())
                        );
        }
        ...
        if constexpr (multi_gpu) {
          ...
          edge_exists = cur_graph_view.has_edge(
                          handle,
                          raft::device_span<vertex_t const>(
                            std::get<4>(triangles_from_weak_edges).data(), intersection_indices.size()),
                          raft::device_span<vertex_t const>(
                            std::get<5>(triangles_from_weak_edges).data(), intersection_indices.size())
                        );
        }
