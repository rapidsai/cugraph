...
struct call_e_op_t {
  ...
  typename GraphViewType::vertex_type const* indices{nullptr};  // indices = edge_partition.incies() + edge_offset
  typename GraphViewType::edge_type edge_offset{};

  __device__ auto operator()(typename GraphViewType::edge_type i  /* index in key's neighbor list */) const
  {
    auto minor        = indices[i];
    auto minor_offset = edge_partition.minor_offset_from_minor_nocheck(minor);
    std::conditional_t<GraphViewType::is_storage_transposed, typename GraphViewType::vertex_type, key_t> key_or_src{};
    std::conditional_t<GraphViewType::is_storage_transposed, key_t, typename GraphViewType::vertex_type> key_or_dst{};
...
struct call_e_op_with_key_t {
  ...
  __device__ auto operator()(key_t key, typename GraphViewType::edge_type i /* index in edge_partition's edge list */) const
  {
