...
// destination, and multi-edge index triplets; the latter for multi-graphs. The caller can
// optionally tag edges with a user-defined tag_t type object (tag_t should be an arithmetic type).
// Note that not all primitives taking an edge_bucket_t object support edge_bucket_t with non-void
// tag_t (e.g. transform_e). If sorted_unique is true, stores unique keys in the sorted
// (non-descending) order. If false, there can be duplicates and the keys may not be sorted. Use
// source as the primary key and destination as the secondary key for sorting if src_major is true.
// Use destination as the primary key and source as the secondary key if src_major is false.
// Multi-edge indices (if releveant) are used as the tertiary keys. If tag_t is not void, use tag as
// the last key for sorting.
...
class edge_bucket_t {
 public:
  static bool constexpr is_tagged        = !std::is_same_v<tag_t, void>;
   ...
   */
  template <typename tag_type = tag_t, std::enable_if_t<!std::is_same_v<tag_type, void>>* = nullptr>
  void insert(vertex_t src, vertex_t dst, std::optional<edge_t> multi_edge_index, tag_type tag)
  {
    CUGRAPH_EXPECTS(multi_edge_indices_.has_value() == multi_edge_index.has_value(),
                    "Invalid input argument: multi_edge_index.has_value() does not match with "
                    "multi_edge_indices_.has_value()");
   ...
   * @param multi_edge_index_first Iterator pointing to the first (inclusive) element of the
   * multi-edge indices in device memory (for multi-graphs).
   * @param tag_first Iterator pointing to the first (inclusive) element of the edge tags in
   * device memory.
   */
  template <typename VertexIterator,
            typename MultiEdgeIndexIterator,
              ...
              TagIterator tag_first)
  {
    ...
    if (majors_.size() > 0) {
      if constexpr (sorted_unique) {
        ...
        size_t new_size{0};
        if (multi_edge_index_first) {
          auto new_quadreplet_first = thrust::make_zip_iterator(
            thrust::make_tuple(major_first, minor_first, *multi_edge_index_first, tag_first));
          auto old_quadreplet_first = thrust::make_zip_iterator(thrust::make_tuple(
            majors_.begin(), minors_.begin(), multi_edge_indices_->begin(), tags_.begin()));
          auto merged_quadreplet_first =
            thrust::make_zip_iterator(thrust::make_tuple(merged_majors.begin(),
                                                         merged_minors.begin(),
                                                         merged_multi_edge_indices->begin(),
                                                         merged_tags.begin()));
          thrust::merge(handle_ptr_->get_thrust_policy(),
                        old_quadreplet_first,
                        old_quadreplet_first + majors_.size(),
                        new_quadreplet_first,
                        new_quadreplet_first + cuda::std::distance(major_first, major_last),
                        merged_quadreplet_first);
          new_size = static_cast<size_t>(
            cuda::std::distance(merged_quadreplet_first,
                                thrust::unique(handle_ptr_->get_thrust_policy(),
                                               merged_quadreplet_first,
                                               merged_quadreplet_first + merged_majors.size())));
        } else {
          auto new_triplet_first =
        ...
        merged_majors.resize(new_size, handle_ptr_->get_stream());
        merged_minors.resize(new_size, handle_ptr_->get_stream());
        merged_tags.resize(new_size, handle_ptr_->get_stream());
        merged_majors.shrink_to_fit(handle_ptr_->get_stream());
        merged_minors.shrink_to_fit(handle_ptr_->get_stream());
