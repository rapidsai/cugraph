  ...
  graph_view_t<NodeTypeT, EdgeTypeT, WeightT, false, false> const& gview)
{
  ...
  graph.offsets = const_cast<EdgeTypeT*>(gview.local_edge_partition_view().get_offsets());
  graph.indices = const_cast<EdgeTypeT*>(gview.local_edge_partition_view().get_indices());
  return graph;
}
