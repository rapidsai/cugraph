...
{
  std::tie(d_vertices, std::ignore) = cugraph::groupby_gpu_id_and_shuffle_values(
    handle.get_comms(),
    d_vertices.begin(),
    d_vertices.end(),
...
shuffle_vertex_value_pairs_by_gpu_id_impl(raft::handle_t const& handle,
                                          rmm::device_uvector<vertex_t>&& d_vertices,
                                          dataframe_buffer_type_t<value_t>&& d_values,
                                          func_t func)
{
  std::tie(d_vertices, d_values, std::ignore) = cugraph::groupby_gpu_id_and_shuffle_kv_pairs(
    ...
    d_vertices.begin(),
    d_vertices.end(),
    get_dataframe_buffer_begin(d_values),
    [key_func = func] __device__(auto val) { return key_func(val); },
    handle.get_stream());
  ...
  rmm::device_uvector<value_t>&& values)
{
  ...
  std::tie(d_vertices, d_values) = shuffle_vertex_value_pairs_by_gpu_id_impl(
    handle,
    std::move(vertices),
    std::move(values),
    cugraph::detail::compute_gpu_id_from_ext_vertex_t<vertex_t>{
  ...
  std::vector<vertex_t> const& vertex_partition_range_lasts)
{
  ...
  std::tie(d_vertices, d_values, std::ignore) = shuffle_vertex_value_pairs_by_gpu_id_impl(
    handle,
    std::move(vertices),
    std::move(values),
