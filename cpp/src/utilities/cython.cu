 ...
 */
                                                                ...
                                                                bool scramble_vertex_ids)
 {
   auto src_dst_tuple = cugraph::experimental::generate_rmat_edgelist<vertex_t>(
     handle, scale, num_edges, a, b, c, seed, clip_and_flip, scramble_vertex_ids);  // double, not
                                                                                    // triplet
...
std::vector<std::pair<std::unique_ptr<rmm::device_uvector<vertex_t>>, std::unique_ptr<rmm::device_uvector<vertex_t>>>> call_generate_rmat_edgelists(
   ...
   bool scramble_vertex_ids)
 {
   ...
   std::vector<std::pair<std::unique_ptr<rmm::device_uvector<vertex_t>>, std::unique_ptr<rmm::device_uvector<vertex_t>>>> gg_vals;
   ...
   std::transform(src_dst_vec_tuple.begin(), src_dst_vec_tuple.end(),
               std::back_inserter(gg_vals),
               [](auto tpl_dev_uvec){
                  return std::make_pair(std::make_unique<rmm::device_uvector>(std::move(std::get<0>(tpl_dev_uvec))),
                  std::make_unique<rmm::device_uvector>(std::move(std::get<1>(tpl_dev_uvec))));
               });
 ...
 template std::vector<std::pair<std::unique_ptr<rmm::device_uvector<int32_t>>, std::unique_ptr<rmm::device_uvector<int32_t>>>> call_generate_rmat_edgelists<int32_t>(
   raft::handle_t const& handle,
   size_t n_edgelists,
   size_t min_scale,
   size_t max_scale,
 ...
 template std::vector<std::pair<std::unique_ptr<rmm::device_uvector<int64_t>>, std::unique_ptr<rmm::device_uvector<int64_t>>>> call_generate_rmat_edgelists<int64_t>(
   raft::handle_t const& handle,
   size_t n_edgelists,
   size_t min_scale,
   size_t max_scale,
