     ...
     if (nnz<1200000000)
     {
       #pragma omp parallel num_threads(1)
       {
        ...
        #pragma omp master
        {
          ...
          CUDA_RT_CALL(cudaMemcpy(&degree_h[0], col_x[0]->data,   sizeof(idx_t) * m, cudaMemcpyDeviceToHost));
      ...
      #pragma omp parallel num_threads(n_gpus)
       {
        ...
        #pragma omp master
        {
          ...
          for (auto j = 0; j < degree_h.size(); ++j)
            EXPECT_LE(degree_ref[j], degree_h[j]);
        }
...
INSTANTIATE_TEST_CASE_P(hibench_test, Tests_MGDegree_hibench,
                        ::testing::Values(   MGDegree_Usecase("benchmark/hibench/1/Input-small/edges/part-00000", 0)
                                            ...
                                            ,MGDegree_Usecase("benchmark/hibench/1/Input-huge/edges/part-00000", 0)
