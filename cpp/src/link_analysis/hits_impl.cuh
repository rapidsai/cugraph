...
hits(raft::handle_t const& handle,
  ...
  rmm::device_uvector<result_t> curr_authorities(graph_view.get_number_of_local_vertices(), handle.get_stream());
  col_properties_t<GraphViewType, result_t> curr_dst_auth(handle, graph_view);
  rmm::device_uvector<result_t> prev_hubs(graph_view.get_number_of_local_vertices(), handle.get_stream());
  rmm::device_uvector<result_t> curr_hubs(graph_view.get_number_of_local_vertices(), handle.get_stream());
  ...
  for (size_t iter = 0; iter < max_iterations; ++iter) {
    ...
    copy_v_transform_reduce_out_nbr(
      ...
      [] __device__(auto, auto, auto, auto prev_src_hub_value, auto) {
  ...
  return std::make_tuple(std::move(curr_hubs), std::move(curr_authorities), hubs_error, iteration_count);
}
