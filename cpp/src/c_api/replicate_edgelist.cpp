...
cugraph_error_code_t cugraph_replicate_edgelist(
    ...
    cugraph::c_api::cugraph_error_t** error){
        ...
        std::optional<rmm::device_uvector<weight_t>> edgelist_weights =
            weights
            ? std::make_optional(rmm::device_uvector<weight_t>(weights->size_, handle.get_stream()))
            : std::nullopt;
        
        if (edgelist_weights) {
            raft::copy<weight_t>(edgelist_weights->data(),
                                weights->as_type<weight_t>(),
                                weights->size_,
                                handle.get_stream());
        }
        ...
        try{
            ...
            if (edgelist_weights){
                auto gathered_edgelist_weights = cugraph::c_api::detail::device_allgatherv(
                    handle, raft::device_span<weight_t const>(edgelist_weights->data(), edgelist_weights->size()));
                ...
                *result = new cugraph::c_api::cugraph_induced_subgraph_result_t{
                    ...
                    new cugraph::c_api::cugraph_type_erased_device_array_t(gathered_edgelist_weights, weights->type_),
                    new cugraph::c_api::cugraph_type_erased_device_array_t(edge_offsets, cugraph_data_type_id_t::SIZE_T)};
            } else {
                *result = new cugraph::c_api::cugraph_induced_subgraph_result_t{
                    new cugraph::c_api::cugraph_type_erased_device_array_t(gathered_edgelist_srcs, src->type_),
                    new cugraph::c_api::cugraph_type_erased_device_array_t(gathered_edgelist_dsts, dst->type_),
