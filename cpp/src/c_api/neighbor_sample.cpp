...
struct neighbor_sampling_functor : public cugraph::c_api::abstract_functor {
  ...
  void operator()()
  {
    ...
    if constexpr (!cugraph::is_candidate<vertex_t, edge_t, weight_t>::value) {
      ...
      auto const is_temporal = options_.temporal_sampling_enabled_ == TRUE;
      ...
      if (is_temporal && edge_start_times == nullptr) {
      ...
      std::optional<cugraph::temporal_sampling_comparison_t> temporal_sampling_comparison{
        std::nullopt};
      if (is_temporal) {
      ...
      std::tie(sampled_edge_srcs,
        ...
        neighbor_sample<vertex_t, edge_t, weight_t, edge_type_t, time_stamp_t, false, multi_gpu>(
          ...
          (is_temporal) ? std::make_optional(edge_start_times->view()) : std::nullopt,
          (is_temporal && edge_end_times != nullptr) ? std::make_optional(edge_end_times->view())
                                                     : std::nullopt,
          edge_bias_view,
          ...
          cugraph::sampling_options_t{options_.prior_sources_behavior_,
  ...
  cugraph_error_t** error)
{
  ...
  CAPI_EXPECTS(
    ...
    CUGRAPH_INVALID_INPUT,
    "seed time windows require temporal sampling; call "
