...
void pagerank_this_partition(
  ...
  VertexIterator personalization_vertex_first,
  VertexIterator personalization_vertex_last,
  ...
  bool do_expensive_check = false)
{
  ...
  if (do_expensive_check) {
    auto num_nonpositive_weight_sums = count_if_adj_matrix_col(
      handle, csc_graph, adj_matrix_col_out_weight_sum_first, [] __device__(auto val) {
        return val < static_cast<result_t>(0.0);
      });
    CUGRAPH_EXPECTS(num_nonpositive_weight_sums == 0,
    ...
    if (has_initial_guess) {
      auto num_negative_values = count_if_v(
        handle, csc_graph, pagerank_first, [] __device__(auto val) { return val < 0.0; });
    ...
    if (personalize) {
      auto num_negative_values =
        thrust::count_if_v(handle, csc_graph, personalization_value_first, [] __device__(auto val) {
          return val < 0.0;
        });
      CUGRAPH_EXPECTS(num_negative_values == 0,
  ...
  if (has_initial_guess) {
    ...
    thrust::transform(pagerank_first,
                      pagerank_first + num_this_partition_vertices,
                      pagerank_first,
                      [sum] __device__(auto val) { return val / sum; });
  } else {
    thrust::fill(pagerank_first,
                 pagerank_first + num_this_partition_vertices,
  ...
  rmm::device_vector<result_t> adj_matrix_col_pageranks(num_adj_matrix_col_vertices,
                                                        static_cast<result_t>(0.0));
  size_t iter = 0;
  while (true) {
    copy_to_adj_matrix_col(handle, csc_graph, pagerank_first, adj_matrix_col_pageranks.begin());

    auto col_val_first = thrust::make_zip_iterator(
      thrust::make_tuple(adj_matrix_col_pageranks.begin(), adj_matrix_col_out_weight_sum_first));
    thrust::transform(col_val_first,
                      col_val_first + num_this_partition_adj_matrix_col_vertices,
                      adj_matrix_col_pageranks.begin(),
                      [] __device__(auto val) {
                        auto const col_pagerank       = thrust::get<0>(val);
                        auto const col_out_weight_sum = thrust::get<1>(val);
                        auto const divisor = col_out_weight_sum == static_cast<result_t>(0.0)
                                               ? static_cast<result_t>(1.0)
                                               : col_out_weight_sum;
                        return col_pagerank / divisor;
                      });

    auto dangling_sum = transform_reduce_v_with_adj_matrix_col(
      handle,
      csc_graph,
      thrust::make_constant_iteraotr(0) /* dummy */,
      col_val_first,
      [] __device__(auto v_val, auto col_val) {
        ...
        return col_out_weight_sum == static_cast<result_t>(0.0) ? col_pagerank
                                                                : static_cast<result_t>(0.0);
      },
      static_cast<result_t>(0.0));
    ...
    if (personalize) {
      ...
      thrust::for_each(
        val_first,
        val_first + num_personalization_values,
        [pagerank_first, dangling_sum, personalization_sum, this_partition_vertex_first] __device__(
          auto val) {
...
template void pagerank_this_partition(raft::Handle handle,
                                      ...
                                      uint32_t* personalization_vertex_first,
                                      uint32_t* personalization_vertex_last,
