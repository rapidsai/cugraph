...
void degree_from_vertex_ids(const raft::handle_t &handle,
  ...
  if (handle.comms_initialized()) {
    ...
    comm.allreduce(degree, degree, number_of_vertices, raft::comms::op_t::SUM,
                   stream);
  }
}
...
template <typename VT, typename ET, typename WT>
void GraphCompressedSparseBaseView<VT, ET, WT>::get_source_indices(
...
template <typename VT, typename ET, typename WT>
void GraphCOOView<VT, ET, WT>::degree(ET *degree,
  ...
  if (direction != DegreeDirection::IN) {
    if (GraphViewBase<VT, ET, WT>::handle
  ...
  if (direction != DegreeDirection::OUT) {
    degree_from_vertex_ids(GraphViewBase<VT, ET, WT>::handle[0],
                           GraphViewBase<VT, ET, WT>::number_of_vertices,
                           GraphViewBase<VT, ET, WT>::number_of_edges,
                           dst_indices, degree, stream);
  }
}

template <typename VT, typename ET, typename WT>
void GraphCompressedSparseBaseView<VT, ET, WT>::degree(
  ...
  if (direction != DegreeDirection::IN) {
    ...
    degree_from_offsets(GraphViewBase<VT, ET, WT>::number_of_vertices, offsets,
                        degree, stream);
  }
  ...
  if (direction != DegreeDirection::OUT) {
    degree_from_vertex_ids(GraphViewBase<VT, ET, WT>::handle[0],
                           GraphViewBase<VT, ET, WT>::number_of_vertices,
                           GraphViewBase<VT, ET, WT>::number_of_edges, indices,
                           degree, stream);
  }
}
