...
struct rrandom_gen_t {
  ...
  {
    thrust::transform_if(
      rmm::exec_policy(handle_.get_stream()),
      d_ptr_random_,
...
struct random_walker_t {
             ...
             device_vec_t<index_t>& d_sizes) const  // out: init sizes to {1,...}
  {
    ...
    //
    thrust::copy_n(rmm::exec_policy(handle_.get_stream()),
    ...
    thrust::scatter(rmm::exec_policy(handle_.get_stream()),
                    d_src_init_v.begin(),
                    d_src_init_v.end(),
                    map_it_begin,
                    d_paths_v_set.begin());
  ...
  bool all_paths_stopped(device_vec_t<edge_t> const& d_crt_out_degs) const
  {
    auto how_many_stopped =
      thrust::count_if(rmm::exec_policy(handle_.get_stream()),
            ...
            device_vec_t<index_t> const& d_sizes) const  // paths sizes
  {
    ...
    auto new_end_v = thrust::remove_if(rmm::exec_policy(handle_.get_stream()),
                                       d_coalesced_v.begin(),
                                       d_coalesced_v.end(),
                                       thrust::make_counting_iterator<index_t>(0),
                                       predicate_v);

    auto new_end_w = thrust::remove_if(rmm::exec_policy(handle_.get_stream()),
                                       d_coalesced_w.begin(),
                                       d_coalesced_w.end(),
                                       thrust::make_counting_iterator<index_t>(0),
                                       predicate_w);
    ...
    index_t nelems) const  // nelems = number of elements to gather (typically num_paths_)
  {
    ...
    thrust::gather(rmm::exec_policy(handle_.get_stream()),
                   map_it_begin,
                   map_it_begin + nelems,
                   d_src.begin(),
                   d_result.begin());
    ...
    const  // adjusting parameter for scattering vertices (0) or weights (1); see above for more;
  {
    ...
    thrust::scatter_if(rmm::exec_policy(handle_.get_stream()),
                       d_src.begin(),
                       d_src.end(),
                       map_it_begin,
                       d_crt_out_degs.begin(),
                  ...
                  seeding_policy_t seeder = clock_seeding_t<typename random_engine_t::seed_type>{})
{
  ...
  auto how_many_valid = thrust::count_if(rmm::exec_policy(handle.get_stream()),
                                         d_v_start.begin(),
                                         d_v_start.end(),
...
struct coo_convertor_t {
    ...
    device_const_vector_view<index_t>& d_sizes) const
  {
    ...
    //  anything):
    //
    auto new_end_it = thrust::copy_if(rmm::exec_policy(handle_.get_stream()),
                                      d_sizes.begin(),
                                      d_sizes.end(),
                                      d_sz_w_scan.begin(),
    ...
    device_const_vector_view<index_t>& d_sizes) const
  {
    ...
    thrust::inclusive_scan(
      rmm::exec_policy(handle_.get_stream()), d_sizes.begin(), d_sizes.end(), d_scan.begin());
    ...
    index_t total_sz_v) const
  {
    ...
    //
    thrust::copy_if(rmm::exec_policy(handle_.get_stream()),
                    thrust::make_counting_iterator<index_t>(0),
...
query_rw_sizes_offsets(raft::handle_t const& handle, index_t num_paths, index_t const* ptr_d_sizes)
{
  ...
  thrust::exclusive_scan(rmm::exec_policy(handle.get_stream()),
                         ptr_d_sizes,
