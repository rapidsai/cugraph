                           ...
                           std::optional<rmm::device_uvector<int32_t>>&& result_labels)
{
  ...
  arithmetic_device_uvector_t discarded_tmp_indices{std::monostate{}};
  ...
  //    Sort by minor vertex id and identify duplicates.  local_positions is a unique per-row
  ...
  if (keep_labels) {
    thrust::sort(
      ...
      thrust::make_zip_iterator(keep_labels->begin(), keep_minors.begin(), local_positions.begin()),
      thrust::make_zip_iterator(keep_labels->end(), keep_minors.end(), local_positions.end()));
  } else {
    thrust::sort(handle.get_thrust_policy(),
                 thrust::make_zip_iterator(keep_minors.begin(), local_positions.begin()),
                 thrust::make_zip_iterator(keep_minors.end(), local_positions.end()));
  }
  ...
  if (global_remove_count > 0) {
    // Gather to reflect the original positions of the edges (sorted row -> original local index).
    {
    ...
    // Discarded edges: sorted-shard rows that are not first-in-(label, minor), mapped to original
    // local indices in the caller's result_* buffers (lockstep with tmp_edge_indices / labels).
    {
      size_t const n_sorted                = keep_minors.size();
      size_t const duplicate_discard_count = n_sorted - keep_count;
      if (duplicate_discard_count > 0) {
        rmm::device_uvector<uint32_t> dup_flags(cugraph::packed_bool_size(n_sorted),
                                                handle.get_stream());
        raft::copy(dup_flags.begin(), keep_flags.begin(), dup_flags.size(), handle.get_stream());
        detail::invert_flags(
          handle, raft::device_span<uint32_t>{dup_flags.data(), dup_flags.size()}, n_sorted);

        rmm::device_uvector<size_t> sorted_orig(n_sorted, handle.get_stream());
        raft::copy(sorted_orig.begin(), keep_positions.begin(), n_sorted, handle.get_stream());

        rmm::device_uvector<size_t> discarded_orig_idx = detail::keep_marked_entries(
          handle,
          std::move(sorted_orig),
          raft::device_span<uint32_t const>{dup_flags.data(), dup_flags.size()},
          duplicate_discard_count);

        discarded_majors.resize(duplicate_discard_count, handle.get_stream());
        thrust::gather(handle.get_thrust_policy(),
                       discarded_orig_idx.begin(),
                       discarded_orig_idx.end(),
                       result_majors.begin(),
                       discarded_majors.begin());

        discarded_minors.resize(duplicate_discard_count, handle.get_stream());
        thrust::gather(handle.get_thrust_policy(),
                       discarded_orig_idx.begin(),
                       discarded_orig_idx.end(),
                       result_minors.begin(),
        ...
        if (result_labels) {
          rmm::device_uvector<int32_t> dl(duplicate_discard_count, handle.get_stream());
          thrust::gather(handle.get_thrust_policy(),
                         discarded_orig_idx.begin(),
                         discarded_orig_idx.end(),
                         result_labels->begin(),
                         dl.begin());
          discarded_labels = std::move(dl);
        }

        if (!std::holds_alternative<std::monostate>(tmp_edge_indices)) {
          discarded_tmp_indices =
            cugraph::variant_type_dispatch(tmp_edge_indices, [&](auto& property) {
              using prop_vec_t = std::remove_reference_t<decltype(property)>;
              using elem_t     = typename prop_vec_t::value_type;
              rmm::device_uvector<elem_t> gathered(duplicate_discard_count, handle.get_stream());
              thrust::gather(handle.get_thrust_policy(),
                             discarded_orig_idx.begin(),
                             discarded_orig_idx.end(),
                             property.begin(),
                             gathered.begin());
              return arithmetic_device_uvector_t{std::move(gathered)};
            });
        }
      }
    }
  ...
  return std::make_tuple(std::move(result_majors),
                         ...
                         std::move(discarded_tmp_indices),
