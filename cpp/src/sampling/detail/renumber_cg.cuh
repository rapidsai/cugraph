...
class renumber_cg_store_t {
 public:
  ...
  /**
   * @brief Bulk contains check with CG parallel probing
   */
  template <typename KeyIterator, typename OutputIterator>
  void contains(KeyIterator key_first,
                KeyIterator key_last,
                ...
                rmm::cuda_stream_view stream)
  {
    ...
    cuco_store_->contains(key_first, key_last, output_first, stream.value());
  }
