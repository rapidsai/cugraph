...
simple_gather_one_hop_multigraph(
...
simple_gather_one_hop_not_multigraph(
  raft::handle_t const& handle,
  graph_view_t<vertex_t, edge_t, false, multi_gpu> const& graph_view,
  raft::device_span<vertex_t const> active_majors,
  ...
  bool do_expensive_check)
{
  ...
  if (edge_property_views.size() == 0) {
    ...
    if (gather_flags) {
      ...
      rmm::device_uvector<edge_type_t> tmp(result_srcs.size(), handle.get_stream());
  ...
  if (edge_property_views.size() == 0) {
    ...
    if (active_major_labels) {
      ...
      if (graph_view.is_multigraph()) {
        ...
        std::tie(result_srcs, result_dsts, tmp_positions) = simple_gather_one_hop_not_multigraph(
          handle,
          graph_view,
          active_majors,
          std::make_optional(raft::device_span<size_t const>{vertex_label_time_positions.data(),
      ...
      if (graph_view.is_multigraph()) {
        ...
        std::tie(result_srcs, result_dsts, tmp_times) =
          simple_gather_one_hop_not_multigraph(handle,
                                               graph_view,
                                               active_majors,
                                               std::make_optional(active_major_times),
                                               do_expensive_check);
    ...
    auto [keep_count, marked_entries] = detail::mark_entries(
      ...
      [d_tmp           = edge_times.data(),
       ...
       gather_flags = *gather_flags] __device__(auto index) {
