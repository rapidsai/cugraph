...
uniform_neighbor_sample_impl(
  ...
  rmm::device_uvector<vertex_t>& d_in,
  std::optional<rmm::device_uvector<vertex_t>>& d_labels,
  raft::host_span<const int> h_fan_out,
  bool with_replacement,
  uint64_t seed)
{
  ...
  auto d_result_weight =
    edge_weight_view ? std::make_optional(rmm::device_uvector<weight_t>(0, handle.get_stream())) : std::nullopt;
  auto d_result_edge_type =
    edge_type_view ? std::make_optional(rmm::device_uvector<weight_t>(0, handle.get_stream())) : std::nullopt;
  rmm::device_uvector<int32_t> d_result_hop(0, handle.get_stream());
  auto d_result_label =
    d_labels ? std::make_optional(rmm::device_uvector<int32_t>(0, handle.get_stream())) : std::nullopt;
  ...
  uint64_t seed)
{
  ...
  return detail::uniform_neighbor_sample_impl(handle,
                                              ...
                                              edge_type_view,
                                              d_start_vs,
                                              std::nullopt, // need to make an optional device vector here...
                                              fan_out,
                                              with_replacement,
                                              seed);
}
