...
__device__
void write_to_frontier(
    vertex_t * thread_frontier,
  ...
  if (threadIdx.x == 0) {
    *block_write_offset = cugraph::detail::traversal::atomicAdd(
        ...
        static_cast<edge_t>(*block_frontier_count));
  }
  __syncthreads();

  //Write block frontier to global memory
  for (int i = threadIdx.x;
      i < (*block_frontier_count);
      i += blockDim.x) {
    output_frontier[(*block_write_offset) + i] = block_frontier[i];
  }
}

template <int BlockSize, int EdgesPerThread,
  typename vertex_t,
  typename edge_t,
  typename Operator>
__global__ void block_per_vertex(
    edge_t * offsets,
    vertex_t * indices,
...
template <int BlockSize, int EdgesPerThread,
  ...
  typename edge_t,
  typename Operator>
__global__ void kernel_per_vertex(
    edge_t * offsets,
    vertex_t * indices,
    vertex_t * input_frontier,
    vertex_t input_frontier_count,
  ...
  while (current_vertex_index < input_frontier_count) {
    ...
    int i = 0;
    int thread_frontier_count = 0;
    for (edge_t block_offset = beg_block_offset;
        block_offset < end_block_offset;
        block_offset += stride) {
...
void large_vertex_lb(cugraph::GraphCSRView<VT, ET, WT> const &graph,
                     ...
                     cudaStream_t stream)
{
  if (bucket.numberOfVertices != 0) {
    ...
    kernel_per_vertex<block_size, 2><<<block_count, block_size, 0, stream>>>(
      graph.offsets, graph.indices,
      bucket.vertexIds, bucket.numberOfVertices,
      vertex_begin,
...
template <typename VT, typename ET, typename WT, typename Operator>
void medium_vertex_lb(cugraph::GraphCSRView<VT, ET, WT> const &graph,
                      DegreeBucket<VT, ET> &bucket,
                      Operator op,
                      VT vertex_begin,
                      VT *output_vertex_ids,
                      ...
                      cudaStream_t stream)
{
  ...
  if (block_count != 0) {
    block_per_vertex<block_size, 2><<<block_count, block_size, 0, stream>>>(
      graph.offsets, graph.indices,
      bucket.vertexIds, bucket.numberOfVertices,
      vertex_begin,
      output_vertex_ids, output_vertex_ids_offset, op);
...
template <typename VT, typename ET, typename WT, typename Operator>
void small_vertex_lb(cugraph::GraphCSRView<VT, ET, WT> const &graph,
                     DegreeBucket<VT, ET> &bucket,
                     Operator op,
                     VT vertex_begin,
                     VT *output_vertex_ids,
                     ...
                     cudaStream_t stream)
{
  ...
  if (bucket.ceilLogDegreeEnd < 6) {
    block_size = 32;
    block_per_vertex<32, 8><<<block_count, block_size, 0, stream>>>(
      graph.offsets, graph.indices, bucket.vertexIds, bucket.numberOfVertices,
      vertex_begin,
      output_vertex_ids, output_vertex_ids_offset, op);
  } else if (bucket.ceilLogDegreeEnd < 8) {
    block_size = 64;
    block_per_vertex<64, 8><<<block_count, block_size, 0, stream>>>(
      graph.offsets, graph.indices, bucket.vertexIds, bucket.numberOfVertices,
      vertex_begin,
      output_vertex_ids, output_vertex_ids_offset, op);
  } else if (bucket.ceilLogDegreeEnd < 10) {
    block_size = 128;
    block_per_vertex<128, 8><<<block_count, block_size, 0, stream>>>(
      graph.offsets, graph.indices, bucket.vertexIds, bucket.numberOfVertices,
      vertex_begin,
      output_vertex_ids, output_vertex_ids_offset, op);
  } else if (bucket.ceilLogDegreeEnd < 12) {
    block_size = 512;
    block_per_vertex<512, 4><<<block_count, block_size, 0, stream>>>(
      graph.offsets, graph.indices, bucket.vertexIds, bucket.numberOfVertices,
      vertex_begin,
      output_vertex_ids, output_vertex_ids_offset, op);
  } else {
