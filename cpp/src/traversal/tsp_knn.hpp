...
*/
void findKneighbors(
  int numPackages, int k, float **x, float **y, int **neighbors, bool useQuadrants)
{
  ...
  for (int i = 0; i < num_bins; ++i) { binoffsets[i + 1] = bincounts[i] + binoffsets[i]; }
  for (int i = 0; i < numPackages; ++i) {
...
#define binloop(idx)                                                  \
  candidates += (binoffsets[idx + 1] - binoffsets[idx]);              \
  ...
  for (int i = 0; i < numPackages; ++i) {
    ...
    float xi = (*x)[i];
    float yi = (*y)[i];
    for (int j = 0; j < k; j++) { nearest[j] = 1e6; }
