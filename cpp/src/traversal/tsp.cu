...
template <typename vertex_t, typename edge_t, typename weight_t>
float TSP<vertex_t, edge_t, weight_t>::check_input() {
  RAFT_EXPECTS(v_ > 0, "0 vertices");
  RAFT_EXPECTS(e_ > 0, "0 edges");
  RAFT_EXPECTS(src_ != nullptr, "Null sources.");
  RAFT_EXPECTS(dst_ != nullptr, "Null destinations.");
  RAFT_EXPECTS(weights_ != nullptr, "Null weights.");
  RAFT_EXPECTS(restarts_ > 0, "0 restarts.");
}

template <typename vertex_t, typename edge_t, typename weight_t>
float TSP<vertex_t, edge_t, weight_t>::compute() {
  offsets = (int *)malloc((sizeof(int) * 2));
  offsets[0] = 0;
  offsets[1] = nodes;
  ...
  num_restart_batches = (restarts + restart_batch -1) / restart_batch;
  restart_resid = restarts - (num_restart_batches - 1) * restart_batch;
  printf(" doing %d batches of size %d, with %d tail \n", num_restart_batches-1, restart_batch, restart_resid);
  printf("configuration: %d nodes, %d restarts, %s input\n", nodes, restarts, argv[1]);

  //Tell the cache how we want it to behave
  cudaFuncSetCacheConfig(simulOpt, cudaFuncCachePreferEqual);

  threads = best_thread_count(nodes);
  ...
  rmm::device_vector<int> work(4 * restart_batch * ((3 * nodes + 2 + 31) / 32 * 32));
  int *work_d = work.data().get();

  float *pos = (float *)malloc(sizeof(float) * (nodes + 1) * 2);
  if (pos == NULL) {
  ...
  for (int g = 0; g < ngraphs; g++) {
    ...
    for (int b = 0; b < num_restart_batches; b++) {
      ...
      simulOpt<<<restart_batch, threads, sizeof(int) * threads>>>(
          nodes_, neighbors_, x_pos_ + offsets[g], y_pos_ + offsets[g], work_);
      ...
      if (best < global_best) {
        ...
        copyFromGPU(pos, soln, sizeof(float) * (nodes + 1) * 2);
        cudaDeviceSynchronize();
...
void TSP<vertex_t, edge_t, weight_t>::knn(int num_packages,
                                          int k,
                                          bool use_quadrants) {
