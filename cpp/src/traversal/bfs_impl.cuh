...
struct update_prev_visited_flag_t {
  uint32_t* prev_visited_flags{nullptr};

  __device__ void operator()(vertex_t v)
  {
    auto mask = uint32_t{1} << (v % (sizeof(uint32_t) * 8));
    atomicOr(prev_visited_flags + (v / (sizeof(uint32_t) * 8)), mask);
  }
};

template <typename vertex_t, bool multi_gpu>
struct e_op_t {
  detail::edge_partition_endpoint_property_device_view_t<vertex_t, bool> visited_flags{};
         ...
         bool do_expensive_check)
{
  ...
  auto dst_visited_flags = edge_dst_property_t<GraphViewType, bool>(handle, push_graph_view);
  if constexpr (GraphViewType::is_multi_gpu) {
  ...
  rmm::device_uvector<uint32_t> prev_visited_flags(
    GraphViewType::is_multi_gpu
      ? size_t{0}
      : static_cast<size_t>(push_graph_view.local_vertex_partition_range_size()),
  ...
  while (true) {
    if (direction_optimizing) {
      ...
      e_op.visited_flags = detail::edge_partition_endpoint_property_device_view_t<vertex_t, bool>(
      ...
      if constexpr (GraphViewType::is_multi_gpu) {
        ...
        thrust::for_each(handle.get_thrust_policy(),
                         vertex_frontier.bucket(bucket_idx_cur).begin(),
                         vertex_frontier.bucket(bucket_idx_cur).end(),
                         update_prev_visited_flag_t<vertex_t>{prev_visited_flags.data()});
