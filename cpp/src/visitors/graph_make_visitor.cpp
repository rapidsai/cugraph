  ...
  visit_graph(graph_envelope_t::base_graph_t const& graph)
{
  if constexpr (mg == false) {
    ...
    bool st                    = *static_cast<bool*>(v_args[7]);
    ...
    cugraph::graph_meta_t<vertex_t, edge_t, false> meta{
      num_vertices, st, graph_props, std::nullopt};
    ...
    graph_envelope_t graph_envelope{vertex_tid, edge_tid, weight_tid, mg_id, graph_tid, ep_graph};
...
// EIDir's:
//
template class graph_maker_visitor<int, int, float, true>;
template class graph_maker_visitor<int, int, double, true>;

template class graph_maker_visitor<int, int, float, false>;
template class graph_maker_visitor<int, int, double, false>;

//------

template class graph_maker_visitor<int, long, float, true>;
template class graph_maker_visitor<int, long, double, true>;

template class graph_maker_visitor<int, long, float, false>;
template class graph_maker_visitor<int, long, double, false>;

//------

template class graph_maker_visitor<long, long, float, true>;
template class graph_maker_visitor<long, long, double, true>;

template class graph_maker_visitor<long, long, float, false>;
template class graph_maker_visitor<long, long, double, false>;
...
return_t graph_create(
  DTypes vertex_tid, DTypes edge_tid, DTypes weight_tid, bool mg, erased_pack_t& ep_cnstr)
{
  try {
    ...
    //
    graph_envelope_t g{vertex_tid, edge_tid, weight_tid, false, graph_tid, ep_graph};
