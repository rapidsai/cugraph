...
#if THRUST_VERSION >= 100900
using thrust::cuda_cub::throw_on_error;
...
#ifndef __CUDA_ARCH__
  cached_device_properties(result, device_id);
#else
  uncached_device_properties(result, device_id);
#endif
  return result;
}
...
size_t max_active_blocks(KernelFunction kernel, const size_t CTA_SIZE, const size_t dynamic_smem_bytes)
{
  ...
  int num_blocks;
  cudaError_t error = cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks, kernel, CTA_SIZE, dynamic_smem_bytes);
  throw_on_error(error, "cudaOccupancyMaxActiveBlocksPerMultiprocessor in max_active_blocks");
  return properties.multiProcessorCount * num_blocks;
