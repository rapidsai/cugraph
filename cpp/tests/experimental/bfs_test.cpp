...
void bfs_reference(
    EdgeIterator offset_first, VertexIterator index_first,
  ...
  std::fill(
    predecessor_first, predecessor_first + num_vertices,
    ...
    : graph_file_path_(graph_file_path), source_(source)
  {
    if ((graph_file_path_.length() > 0) && (graph_file_path[0] != '/')) {
      graph_file_full_path_ =
        cugraph::test::get_rapids_dataset_root_dir() + "/" + graph_file_path_;
    } else {
...
class Tests_BFS : public ::testing::TestWithParam<BFS_Usecase> {
 public:
  ...
  void run_current_test(BFS_Usecase const& configuration)
  {
    ...
    ASSERT_TRUE(
      configuration.source_ >= 0 && configuration.source_ <= csr_graph_view.number_of_vertices)
    ...
    CUDA_RT_CALL(
      cudaMemcpy(
        h_offsets.data(), csr_graph_view.offsets, sizeof(edge_t) * h_offsets.size(),
        cudaMemcpyDeviceToHost));
    CUDA_RT_CALL(
      cudaMemcpy(
        h_indices.data(), csr_graph_view.indices, sizeof(vertex_t) * h_indices.size(),
        cudaMemcpyDeviceToHost));


    bfs_reference(
      h_offsets.begin(), h_indices.begin(),
      h_reference_distances.begin(), h_reference_predecessors.begin(),
      csr_graph_view.number_of_vertices, static_cast<vertex_t>(configuration.source_));

    raft::handle_t handle{};

    rmm::device_uvector<vertex_t> d_distances(csr_graph_view.number_of_vertices, handle.get_stream());
    rmm::device_uvector<vertex_t> d_predecessors(csr_graph_view.number_of_vertices, handle.get_stream());

    cudaDeviceSynchronize();  // for consistent performance measurement

    cugraph::experimental::bfs(
      handle, csr_graph_view,
      d_distances.begin(), d_predecessors.begin(), static_cast<vertex_t>(configuration.source_),
      false, std::numeric_limits<size_t>::max(), false);
    ...
    CUDA_RT_CALL(
      cudaMemcpy(
        h_cugraph_distances.data(), d_distances.data(),
        sizeof(vertex_t) * d_distances.size(), cudaMemcpyDeviceToHost));
    CUDA_RT_CALL(
      cudaMemcpy(
        h_cugraph_predecessors.data(), d_predecessors.data(),
        sizeof(vertex_t) * d_predecessors.size(), cudaMemcpyDeviceToHost));
    ...
    EXPECT_TRUE(
      std::equal(
        h_reference_predecessors.begin(), h_reference_predecessors.end(),
