  ...
  bool with_replacement)
{
  ...
  size_t this_gpu_sample_size{0};
  if constexpr (multi_gpu) {
    auto const comm_rank = handle.get_comms().get_rank();
    auto const comm_size = handle.get_comms().get_size();

    this_gpu_sample_size =
      sample_size / static_cast<size_t>(comm_size) +
      (static_cast<size_t>(comm_rank) < static_cast<size_t>(sample_size % comm_size) ? size_t{1}
                                                                                     : size_t{0});
  } else {
    this_gpu_sample_size = sample_size;
  }

  if (with_replacement) {
    // FIXME: need to double check uniform_random_fill generates random numbers in [0, V) (not [0,
    // V])
    mg_sample_buffer.resize(this_gpu_sample_size, handle.get_stream());
    cugraph::detail::uniform_random_fill(handle.get_stream(),
                                         mg_sample_buffer.data(),
                                         mg_sample_buffer.size(),
                                         vertex_t{0},
                                         vertex_partition_range_lasts.back(),
                                         rng_state);
  } else {
    ...
    if constexpr (multi_gpu) {
      ...
      local_vertex_partition_range_last = vertex_partition_range_lasts[vertex_partition_id];
    } else {
      local_vertex_partition_range_first = vertex_t{0};
      local_vertex_partition_range_last  = vertex_partition_range_lasts.back();
    }
    ...
    if constexpr (multi_gpu) {
      ...
      if (min_buffer_size <= sample_size / comm_size) {
        auto new_sizes    = std::vector<size_t>(comm_size, min_buffer_size);
        auto num_deficits = sample_size - min_buffer_size * comm_size;
        for (int i = 0; i < comm_size; ++i) {
          auto delta = std::min(num_deficits, buffer_sizes[i] - min_buffer_size);
          new_sizes[i] += delta;
          num_deficits -= delta;
        }
        this_gpu_sample_size = new_sizes[comm_rank];
      }
    }
