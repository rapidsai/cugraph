...
inline auto make_pool(bool use_max = false, int comm_size = 1)
{
  ...
  double init_alloc_ratio{};
  if (comm_size <= 16) {
    init_alloc_ratio = 0.95;
  }
  else if (comm_size <= 36) {
    init_alloc_ratio = 0.915;
  }
  ...
  auto init_alloc =
    use_max ? rmm::align_down(std::min(free, static_cast<size_t>(total * init_alloc_ratio)),
                              rmm::CUDA_ALLOCATION_ALIGNMENT)
            : rmm::align_down(std::min(free, total / 10), rmm::CUDA_ALLOCATION_ALIGNMENT);
  std::optional<size_t> max_alloc{std::nullopt};
  if (use_max) { max_alloc = init_alloc; }
  std::cout << "init_alloc_ratio=" << static_cast<double>(init_alloc) / static_cast<double>(total)
            << std::endl;
  ...
  std::string const& allocation_mode, int comm_size)
{
  ...
  if (allocation_mode == "pool") return make_pool(false);
...
#define CUGRAPH_TEST_PROGRAM_MAIN()                                                     \
  ...
  {                                                                                     \
    ...
    auto resource       = cugraph::test::create_memory_resource(rmm_mode, int{1});      \
