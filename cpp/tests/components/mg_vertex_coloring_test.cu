  ...
  void run_current_test(std::tuple<GraphColoring_UseCase, input_usecase_t> const& param)
  {
    ...
    if (coloring_usecase.check_correctness) {
      ...
      if constexpr (multi_gpu) {
        ...
        update_edge_src_property(*handle_, mg_graph_view, d_colors.begin(), src_color_cache.mutable_view());
        update_edge_dst_property(*handle_, mg_graph_view, d_colors.begin(), dst_color_cache.mutable_view());
      }
