...
struct hits_result {
  rmm::device_uvector<result_t> hubs;
...
hits_result<result_t> hits_reference(raft::handle_t const& handle,
                                     ...
                                     double tolerance)
{
  std::vector<edge_t> h_offsets(num_vertices + 1);
  std::vector<vertex_t> h_indices(num_edges);
  raft::update_host(h_offsets.data(), offsets, num_vertices + 1, handle.get_stream());
  raft::update_host(h_indices.data(), indices, num_edges, handle.get_stream());
  ...
  rmm::device_uvector<result_t> hubs(curr_hubs.size(), handle.get_stream());
  rmm::device_uvector<result_t> authorities(curr_authorities.size(), handle.get_stream());
  raft::update_device(hubs.data(), curr_hubs.data(), curr_hubs.size(), handle.get_stream());
  raft::update_device(
    authorities.data(), curr_authorities.data(), curr_authorities.size(), handle.get_stream());
  return hits_result<result_t>{
    std::move(hubs), std::move(authorities), hubs_error, hubs_iterations};
}
 ...
 public:
  ...
  void run_current_test(Hits_Usecase const& hits_usecase, input_usecase_t const& input_usecase)
  {
    ...
    auto reference_result =
      hits_reference<weight_t>(handle,
