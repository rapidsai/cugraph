  ...
  vertex_t const* labels)
{
  ...
  return std::make_unique<cugraph::graph_t<vertex_t, edge_t, weight_t, false>>(
    handle,
    edgelist,
...
template std::unique_ptr<cugraph::graph_t<int32_t, int32_t, float, false>> coarsen_graph(
  raft::handle_t const& handle,
  cugraph::graph_view_t<int32_t, int32_t, float, false> const& graph_view,
