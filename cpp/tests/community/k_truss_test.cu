 ...
 public:
  ...
  k_truss_reference(
      ...
      edge_t k)
    {
      ...
      raft::update_host(h_src.data(),
                        d_src.data(),
                        d_src.size(),
                        handle.get_stream());
      std::vector<vertex_t> h_dst(d_src.size());
      raft::update_host(h_dst.data(),
                        d_dst.data(),
                        d_dst.size(),
                        handle.get_stream());
      ...
      auto edge_first =
          thrust::make_zip_iterator(thrust::make_tuple(h_src.begin(), h_dst.begin()));
      ...
      while (n_dropped > 0) {
        ...
        for (auto u=vertices.begin(); u!=vertices.end(); ++u) {
            ...
            vertex_t idx_end = 0;
            auto itr_lower_range = thrust::lower_bound(
                   thrust::host, h_src.begin(), h_src.end(), *u);
        
            idx_start = thrust::distance(h_src.begin(), itr_lower_range);
        
            if (*itr_lower_range == *u) {
              auto itr_upper_range = thrust::upper_bound(
                thrust::host, itr_lower_range, h_src.end(), *u);
            ...
            std::set_difference(nbrs_u.begin(), nbrs_u.end(), seen.begin(), seen.end(),
              std::inserter(new_nbrs, new_nbrs.end()));
            
            // Finding the neighbors of v
            for (auto v=new_nbrs.begin(); v!=new_nbrs.end(); ++v) {
                auto itr_lower_range = thrust::lower_bound(
                            thrust::host, h_src.begin(), h_src.end(), *v);

                idx_start = thrust::distance(h_src.begin(), itr_lower_range);

                if (*itr_lower_range == *v) {
                  auto itr_upper_range = thrust::upper_bound(
                    thrust::host, itr_lower_range, h_src.end(), *v);
                ...
                std::set_intersection(nbrs_u.begin(), nbrs_u.end(), nbrs_v.begin(), nbrs_v.end(), std::inserter(nbr_intersection_u_v, nbr_intersection_u_v.end()));
      ...
      rmm::device_uvector<vertex_t> d_reference_src(h_src.size(),
                                                    handle.get_stream());
      
      raft::update_device(d_reference_src.data(),
                          h_src.data(),
                          h_src.size(),
                          handle.get_stream());
      
      rmm::device_uvector<vertex_t> d_reference_dst(h_src.size(),
                                                    handle.get_stream());
      
      raft::update_device(d_reference_dst.data(),
                          h_dst.data(),
                          h_dst.size(),
                          handle.get_stream());

      auto d_reference_wgt = std::make_optional<rmm::device_uvector<weight_t>>(
        h_src.size(), handle.get_stream());

      if (edge_weight_view) {
        raft::update_device((*d_reference_wgt).data(),
                            h_wgt.data(),
                            h_wgt.size(),
                            handle.get_stream());
      }
  ...
  void run_current_test(std::tuple<KTruss_Usecase const&, input_usecase_t const&> const& param)
  {
    ...
    auto [d_cugraph_src, d_cugraph_dst, d_cugraph_wgt] = cugraph::k_truss<vertex_t, edge_t, weight_t, false>(
    ...
    if (k_truss_usecase.check_correctness) {
      auto [d_reference_src, d_reference_dst, d_reference_wgt] = k_truss_reference<vertex_t, edge_t, weight_t>(handle, graph_view, edge_weight_view, k_truss_usecase.k);
      if (edge_weight_view) {
        auto d_edge_first =
            thrust::make_zip_iterator(thrust::make_tuple(d_cugraph_src.begin(), d_cugraph_dst.begin(), (*d_cugraph_wgt).begin()));
        
        thrust::sort(handle.get_thrust_policy(), d_edge_first, d_edge_first + d_cugraph_src.size());
      } else {
        auto d_edge_first =
            thrust::make_zip_iterator(thrust::make_tuple(d_cugraph_src.begin(), d_cugraph_dst.begin()));
      ...
      if (edge_weight_view) {
        ...
        weight_t{1e-3},
        weight_t{(weight_t{1} / static_cast<weight_t>((*d_cugraph_wgt).size())) * weight_t{1e-3}}};
...
{
  run_current_test<int32_t, int32_t, float>(override_File_Usecase_with_cmd_line_arguments(GetParam()));
}

TEST_P(Tests_KTruss_File, CheckInt64Int64Float)
{
  run_current_test<int64_t, int64_t, float>(override_File_Usecase_with_cmd_line_arguments(GetParam()));
}

TEST_P(Tests_KTruss_File, CheckInt32Int64Float)
{
  run_current_test<int32_t, int64_t, float>(override_File_Usecase_with_cmd_line_arguments(GetParam()));
}
...
INSTANTIATE_TEST_SUITE_P(
  ...
  ::testing::Combine(
    ...
    ::testing::Values(cugraph::test::File_Usecase("/home/nfs/jnke/ktruss/cugraph/datasets/dolphins.mtx"),
