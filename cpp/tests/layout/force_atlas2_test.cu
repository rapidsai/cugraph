...
class Tests_Force_Atlas2 : public ::testing::TestWithParam<Force_Atlas2_Usecase> {
 public:
  ...
  void run_current_test(const Force_Atlas2_Usecase& param)
  {
    ...
    RAFT_CHECK_CUDA(cudaMemcpy(srcs, &cooRowInd[0], sizeof(int) * nnz, cudaMemcpyDefault));
    RAFT_CHECK_CUDA(cudaMemcpy(dests, &cooColInd[0], sizeof(int) * nnz, cudaMemcpyDefault));
    RAFT_CHECK_CUDA(cudaMemcpy(weights, &cooVal[0], sizeof(T) * nnz, cudaMemcpyDefault));
    ...
    RAFT_CHECK_CUDA(
      cudaMemcpy(&h_pos[0], pos.data(), sizeof(float) * m * 2, cudaMemcpyDeviceToHost));
