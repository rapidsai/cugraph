...
std::tuple<vertex_t, int, distance_t, vertex_t> traverse_to_pruned_graph(
  raft::handle_t const handle,
  raft::device_span<vertex_t const> parents /* found in forest pruning */,
  ...
  std::optional<distance_t> weight_to_starting_vertex_parent,
  vertex_t local_vertex_partition_range_first,
  int vertex_partition_id)
{
  assert(weights.has_value() == weight_to_starting_vertex_parent.has_value());
  auto& comm                 = handle.get_comms();
  ...
  auto w              = weight_to_starting_vertex_parent;
  vertex_t unrenumbered_subgraph_starting_vertex_parent{};
  while (true) {
    if (w) {  // SSSP
      subgraph_starting_vertex_distance += *w;
    } else {  // BFS
    ...
    if (n_vertex_partition_id == vertex_partition_id) {
      ...
      raft::update_host(std::addressof(nn),
                        ...
                        handle.get_stream());
      if (w) {  // SSSP
        raft::update_host(std::addressof(*w),
                          weights->data() + (n - local_vertex_partition_range_first),
                          size_t{1},
                          handle.get_stream());
      }
      handle.sync_stream();
    ...
    if (w) {  // SSSP
      thrust::tie(unrenumbered_n, nn, w) = cugraph::host_scalar_bcast(
        comm,
        thrust::make_tuple(unrenumbered_n, nn, *w),
    ...
    if (weights) {  // SSSP
      auto dist_first = thrust::make_transform_iterator(
        ...
        cuda::proclaim_return_type<distance_t>(
          [weights = raft::device_span<distance_t const>(weights->data(), weights->size()),
      ...
      auto dist_first =
        thrust::make_transform_iterator(remaing_vertex_parent_dists.begin() + new_size,
                                        cuda::proclaim_return_type<vertex_t>([] __device__(auto d) {
                                          return static_cast<distance_t>(d + 1);
                                        }));
      thrust::scatter(handle.get_thrust_policy(),
                      dist_first,
                      dist_first + (remaining_vertices.size() - new_size),
