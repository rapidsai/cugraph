  ...
  bool in_2cores)
{
  ...
  if (reachable_from_2cores) {  // exclude the edges in the forest (parents[v] -> v)
    ...
    if (!in_2cores) {  // found BFS predecessor tree may contain edges from v ->
      ...
      std::tie(forest_edge_parents, forest_edge_vertices) = cugraph::detail::
  ...
  std::tie(query_preds, query_vertices) =
    cugraph::detail::shuffle_int_vertex_value_pairs_to_local_gpu_by_vertex_partitioning<vertex_t,
                                                                                        vertex_t>(
      handle,
