...
int test_random_walks_1()
{
  ...
  ret_code = cugraph_make_device_buffer(p_handle, INT32, num_edges, &dbuf_src);
  runtime_assert(ret_code == CUGRAPH_SUCCESS, "src device_buffer creation failed.");

  ret_code = cugraph_make_device_buffer(p_handle, INT32, num_edges, &dbuf_dst);
  runtime_assert(ret_code == CUGRAPH_SUCCESS, "dst device_buffer creation failed.");

  ret_code = cugraph_make_device_buffer(p_handle, FLOAT32, num_edges, &dbuf_wgt);
  runtime_assert(ret_code == CUGRAPH_SUCCESS, "weight device_buffer creation failed.");

  ret_code = cugraph_update_device_buffer(p_handle, INT32, &dbuf_src, (byte_t*)h_src);
  runtime_assert(ret_code == CUGRAPH_SUCCESS, "src device_buffer update failed.");

  ret_code = cugraph_update_device_buffer(p_handle, INT32, &dbuf_dst, (byte_t*)h_dst);
  runtime_assert(ret_code == CUGRAPH_SUCCESS, "dst device_buffer update failed.");

  ret_code = cugraph_update_device_buffer(p_handle, FLOAT32, &dbuf_wgt, (byte_t*)h_wgt);
