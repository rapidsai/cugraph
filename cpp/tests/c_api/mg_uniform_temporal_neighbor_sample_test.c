  ...
  bool_t is_multigraph)
{
  ...
  cugraph_type_erased_device_array_view_t* result_renumber_map         = NULL;
  cugraph_type_erased_device_array_view_t* result_renumber_map_offsets = NULL;
  ...
  result_size = cugraph_test_scalar_reduce(handle, result_size);
  ...
  if (h_edge_start_times != NULL) {
    ret_code =
      cugraph_test_device_gatherv_fill(handle, result_edge_start_times, h_result_edge_start_times);
    TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, "gatherv_fill failed.");
  }

  if (h_edge_end_times != NULL) {
    ret_code =
      cugraph_test_device_gatherv_fill(handle, result_edge_end_times, h_result_edge_end_times);
    TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, "gatherv_fill failed.");
  }
  ...
  if (result_size > 0) {
    ...
    // We're grouped by label (outer) and hop (inner)... so the below loop is not right.
    // Probably need to fix in SG case also.  Might also need to fix in non-temporal case.
    //   Although I could leave the non-temporal case as a FIXME later to allow Joseph to make
    //   progress.
    if (num_start_labels > 1) {
      for (size_t label_id = 0; label_id < (num_start_labels - 1); ++label_id) {
        ...
        if (dedupe_sources) {
          ...
          // destination. We test this by verifying that vertex v only appears in <= hop
          // size/degree
          for (int hop = 1; hop < fan_out_size; ++hop) {
            for (size_t i = h_result_offsets[label_id * fan_out_size + hop];
                 (i < h_result_offsets[label_id * fan_out_size + hop + 1]) && (test_ret_value == 0);
              ...
              for (size_t j = h_result_offsets[label_id * fan_out_size + hop];
                   (j < h_result_offsets[label_id * fan_out_size + hop + 1]) &&
                   (test_ret_value == 0);
                   ++j) {
