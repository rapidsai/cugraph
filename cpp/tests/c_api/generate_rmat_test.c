...
int test_rmat_generation()
{
  ...
  vertex_t expected_src[] = {17, 18, 0,  16, 1,  24, 16, 1, 6, 4,  2, 1, 14, 2,  16,
                             2,  5,  23, 4,  10, 4,  3,  0, 4, 11, 0, 0, 2,  24, 0};
  vertex_t expected_dst[] = {0, 10, 23, 0,  26, 0, 2,  1, 27, 8,  1, 0, 21, 21, 0,
  ...
  ret_code = cugraph_generate_rmat_edgelist(
    handle, rng_state, 5, 30, 0.57, 0.19, 0.19, FALSE, FALSE, &coo, &ret_error);
  ...
  for (int i = 0; i < src_size; ++i) {
    TEST_ASSERT(test_ret_value, expected_src[i] == h_src[i], "generated edges don't match");
    TEST_ASSERT(test_ret_value, expected_dst[i] == h_dst[i], "generated edges don't match");
  }
...
int test_rmat_list_generation()
{
  ...
  ret_code = cugraph_generate_rmat_edgelists(
    handle, rng_state, num_lists, 4, 6, 4, UNIFORM, POWER_LAW, FALSE, FALSE, &coo_list, &ret_error);
  TEST_ASSERT(test_ret_value, ret_code == CUGRAPH_SUCCESS, "generate_rmat_edgelist failed.");
  TEST_ALWAYS_ASSERT(ret_code == CUGRAPH_SUCCESS, cugraph_error_message(ret_error));
  ...
  for (size_t i = 0; (i < num_lists) && (test_ret_value == 0); i++) {
    ...
    for (size_t j = 0; (j < src_size) && (test_ret_value == 0); ++j) {
      TEST_ASSERT(test_ret_value, h_src[j] < max_vertex_id[i], "generated edges don't match");
      TEST_ASSERT(test_ret_value, h_dst[j] < max_vertex_id[i], "generated edges don't match");
    }
