...
template <typename key_t, typename vertex_t, typename output_payload_t, bool store_transposed>
struct e_op_t {
...
class Tests_MGExtractTransformVFrontierIncomingOutgoingE
  : public ::testing::TestWithParam<std::tuple<Prims_Usecase, input_usecase_t>> {
 public:
  Tests_MGExtractTransformVFrontierIncomingOutgoingE() {}
  ...
  // Verify the results of extract_transform_v_frontier_(incoming|outgoing)_e (incoming if
  ...
  void run_current_test(Prims_Usecase const& prims_usecase, input_usecase_t const& input_usecase)
  {
    ...
    // 2. run MG extract_transform_v_frontier_(incoming|outgoing)_e
    ...
    if (cugraph::test::g_perf) {
      RAFT_CUDA_TRY(cudaDeviceSynchronize());  // for consistent performance measurement
      handle_->get_comms().barrier();
      hr_timer.start("MG extract_transform_v_frontier_(incoming|outgoing)_e");
    }
    ...
    if constexpr (store_transposed) {
      mg_extract_transform_output_buffer = cugraph::extract_transform_v_frontier_incoming_e(
        *handle_,
        mg_graph_view,
        mg_vertex_frontier.bucket(bucket_idx_cur),
      ...
      mg_extract_transform_output_buffer = cugraph::extract_transform_v_frontier_outgoing_e(
        *handle_,
        mg_graph_view,
        mg_vertex_frontier.bucket(bucket_idx_cur),
        cugraph::edge_src_dummy_property_t{}.view(),
    ...
    if (prims_usecase.check_correctness) {
      ...
      if (handle_->get_comms().get_rank() == int{0}) {
        ...
        auto sg_extract_transform_output_buffer = cugraph::allocate_dataframe_buffer<
          ...
          size_t{0}, handle_->get_stream());
        if constexpr (store_transposed) {
          sg_extract_transform_output_buffer = cugraph::extract_transform_v_frontier_incoming_e(
            *handle_,
            sg_graph_view,
            sg_vertex_frontier.bucket(bucket_idx_cur),
            cugraph::edge_src_dummy_property_t{}.view(),
          ...
          sg_extract_transform_output_buffer = cugraph::extract_transform_v_frontier_outgoing_e(
            *handle_,
            sg_graph_view,
            sg_vertex_frontier.bucket(bucket_idx_cur),
            cugraph::edge_src_dummy_property_t{}.view(),
...
std::unique_ptr<raft::handle_t>
  Tests_MGExtractTransformVFrontierIncomingOutgoingE<input_usecase_t>::handle_ = nullptr;

using Tests_MGExtractTransformVFrontierIncomingOutgoingE_File =
  Tests_MGExtractTransformVFrontierIncomingOutgoingE<cugraph::test::File_Usecase>;
using Tests_MGExtractTransformVFrontierIncomingOutgoingE_Rmat =
  Tests_MGExtractTransformVFrontierIncomingOutgoingE<cugraph::test::Rmat_Usecase>;

TEST_P(Tests_MGExtractTransformVFrontierIncomingOutgoingE_File,
...
TEST_P(Tests_MGExtractTransformVFrontierIncomingOutgoingE_Rmat,
       CheckInt32Int32FloatVoidInt32TransposeFalse)
{
  auto param = GetParam();
...
INSTANTIATE_TEST_SUITE_P(
  file_test,
  Tests_MGExtractTransformVFrontierIncomingOutgoingE_File,
...
INSTANTIATE_TEST_SUITE_P(rmat_small_test,
                         Tests_MGExtractTransformVFrontierIncomingOutgoingE_Rmat,
