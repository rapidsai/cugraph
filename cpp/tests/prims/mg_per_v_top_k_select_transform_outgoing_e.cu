...
class Tests_MGPerVTopKSelectTransformOutgoingE
  : public ::testing::TestWithParam<std::tuple<Prims_Usecase, input_usecase_t>> {
 public:
  Tests_MGPerVTopKSelectTransformOutgoingE() {}
  ...
  // Verify the results of per_v_top_k_select_transform_outgoing_e primitive
  ...
  void run_current_test(Prims_Usecase const& prims_usecase, input_usecase_t const& input_usecase)
  {
    ...
    auto [mg_graph, mg_edge_weights, mg_renumber_map] =
      cugraph::test::construct_graph<vertex_t, edge_t, weight_t, false, true>(
        *handle_, input_usecase, true /* test_weighted */, true /* renumber */);
    ...
    // 2. run MG per_v_top_k_select_transform_outgoing_e primitive
    ...
    if (cugraph::test::g_perf) {
      RAFT_CUDA_TRY(cudaDeviceSynchronize());  // for consistent performance measurement
      handle_->get_comms().barrier();
      hr_timer.start("MG per_v_top_k_select_transform_outgoing_e");
    }

    auto [mg_sample_offsets, mg_sample_e_op_results] =
      (prims_usecase.Ks.size() == 1)
        ? cugraph::per_v_top_k_select_transform_outgoing_e(
            *handle_,
            mg_graph_view,
            mg_vertex_frontier.bucket(bucket_idx_cur),
            cugraph::edge_src_dummy_property_t{}.view(),
    ...
    if (prims_usecase.check_correctness) {
      ...
      if (handle_->get_comms().get_rank() == 0) {
        ...
        rmm::device_uvector<weight_t> sg_graph_biases(0, handle_->get_stream());
        {
        ...
        rmm::device_uvector<size_t> mg_aggregate_frontier_degree_offsets(
          mg_aggregate_frontier_vertices.size() + 1, handle_->get_stream());
        rmm::device_uvector<bool> mg_aggregate_frontier_nbr_selected_flags(0,
                                                                           handle_->get_stream());
        {
          auto degree_first = thrust::make_transform_iterator(
            mg_aggregate_frontier_vertices.begin(),
            cuda::proclaim_return_type<size_t>(
              [sg_graph_offsets = raft::device_span<edge_t const>(
                 sg_graph_offsets.data(), sg_graph_offsets.size())] __device__(auto v) {
                return static_cast<size_t>(sg_graph_offsets[v + 1] - sg_graph_offsets[v]);
              }));
          mg_aggregate_frontier_degree_offsets.set_element_to_zero_async(0, handle_->get_stream());
          thrust::inclusive_scan(handle_->get_thrust_policy(),
                                 degree_first,
                                 degree_first + mg_aggregate_frontier_vertices.size(),
                                 mg_aggregate_frontier_degree_offsets.begin() + 1);
          mg_aggregate_frontier_nbr_selected_flags.resize(
            mg_aggregate_frontier_degree_offsets.back_element(handle_->get_stream()),
            handle_->get_stream());
          thrust::fill(handle_->get_thrust_policy(),
                       mg_aggregate_frontier_nbr_selected_flags.begin(),
                       mg_aggregate_frontier_nbr_selected_flags.end(),
                       false);
        }
        ...
        auto num_invalids = static_cast<size_t>(thrust::count_if(
          ...
          [frontier_vertex_first = mg_aggregate_frontier_vertices.begin(),
           frontier_degree_offsets =
             raft::device_span<size_t const>(mg_aggregate_frontier_degree_offsets.data(),
                                             mg_aggregate_frontier_degree_offsets.size()),
           frontier_nbr_selected_flags =
             raft::device_span<bool>(mg_aggregate_frontier_nbr_selected_flags.data(),
                                     mg_aggregate_frontier_nbr_selected_flags.size()),
           ...
           sg_graph_biases     = sg_graph_biases.begin(),
           ...
           property_transform =
            ...
            auto sg_nbr_first      = sg_indices + *(sg_graph_offsets + v);
            auto sg_nbr_last       = sg_indices + *(sg_graph_offsets + (v + vertex_t{1}));
            auto sg_nbr_bias_first = sg_graph_biases + *(sg_graph_offsets + v);
            ...
            if (num_edge_types > 1) {
              ...
              auto out_degree = thrust::count_if(thrust::seq,
            ...
            for (size_t j = offset_first; j < offset_last; ++j) {
              ...
              for (auto it = (sg_nbr_bias_first + cuda::std::distance(sg_nbr_first, lower_it));
                   ...
                   ++it) {
                if (*it > 0.0) {  // has a matching positive-bias neighbor
                  found = true;
                  break;
                }
              }
            ...
            // check top-K selection (ties among equal biases may be broken arbitrarily)

            if (sample_count > 0) {
              auto selected_flag_first =
                frontier_nbr_selected_flags.begin() + frontier_degree_offsets[i];
              auto degree = frontier_degree_offsets[i + 1] - frontier_degree_offsets[i];

              if (num_edge_types > 1) {
                assert(sg_graph_edge_types);
                auto sg_nbr_type_first = *sg_graph_edge_types + *(sg_graph_offsets + v);
                for (edge_type_t t = 0; t < num_edge_types; ++t) {
                  // for edges with type t, mark the selected edges; for a multi-edge, pick the
                  // unmarked one with the maximum bias
                  for (size_t j = offset_first; j < offset_last; ++j) {
                    auto e_op_result = *(sample_e_op_result_first + j);
                    auto type =
                      cuda::std::get<cuda::std::tuple_size<result_t>::value - 1>(e_op_result);
                    ...
                    auto upper_it =
                      thrust::upper_bound(thrust::seq, sg_nbr_first, sg_nbr_last, sg_dst);
                    assert((lower_it != sg_nbr_last) && (*lower_it == sg_dst));
                    auto first_o = cuda::std::distance(sg_nbr_first, lower_it);
                    auto last_o  = cuda::std::distance(sg_nbr_first, upper_it);
                    auto best_o  = last_o;
                    for (auto o = first_o; o < last_o; ++o) {
                      if (*(sg_nbr_type_first + o) != t) { continue; }
                      if (*(selected_flag_first + o)) { continue; }
                      if ((best_o == last_o) ||
                          (*(sg_nbr_bias_first + o) > *(sg_nbr_bias_first + best_o))) {
                        best_o = o;
                      }
                    }
                    if (best_o == last_o) { return true; }  // no match
                    *(selected_flag_first + best_o) = true;
                  }

                  // for edges with type t, every unmarked edge must have a bias no larger than the
                  // minimum bias among the marked (selected) edges
                  weight_t min_bias = std::numeric_limits<weight_t>::max();
                  for (edge_t o = 0; o < degree; ++o) {
                    if ((*(sg_nbr_type_first + o) == t) && *(selected_flag_first + o)) {
                      min_bias = cuda::std::min(min_bias, *(sg_nbr_bias_first + o));
                    }
                  }
                  for (edge_t o = 0; o < degree; ++o) {
                    if ((*(sg_nbr_type_first + o) == t) && !*(selected_flag_first + o) &&
                        (*(sg_nbr_bias_first + o) > min_bias)) {
                      return true;
                    }
                  }
                }
              } else {
                // mark the selected edges; for a multi-edge, pick the unmarked one with the maximum
                // bias
                for (size_t j = offset_first; j < offset_last; ++j) {
                  auto sg_dst = cuda::std::get<1>(*(sample_e_op_result_first + j));
                  auto lower_it =
                    thrust::lower_bound(thrust::seq, sg_nbr_first, sg_nbr_last, sg_dst);
                  auto upper_it =
                    thrust::upper_bound(thrust::seq, sg_nbr_first, sg_nbr_last, sg_dst);
                ...
                // every unmarked edge must have a bias no larger than the minimum bias among the
                // marked (selected) edges
                weight_t min_bias = std::numeric_limits<weight_t>::max();
                for (edge_t o = 0; o < degree; ++o) {
                  if (*(selected_flag_first + o)) {
                    min_bias = cuda::std::min(min_bias, *(sg_nbr_bias_first + o));
                  }
                }
                for (edge_t o = 0; o < degree; ++o) {
                  if (!*(selected_flag_first + o) && (*(sg_nbr_bias_first + o) > min_bias)) {
                    return true;
                  }
                }
              }
...
template <typename input_usecase_t>
std::unique_ptr<raft::handle_t> Tests_MGPerVTopKSelectTransformOutgoingE<input_usecase_t>::handle_ =
  nullptr;

using Tests_MGPerVTopKSelectTransformOutgoingE_File =
  Tests_MGPerVTopKSelectTransformOutgoingE<cugraph::test::File_Usecase>;
using Tests_MGPerVTopKSelectTransformOutgoingE_Rmat =
  Tests_MGPerVTopKSelectTransformOutgoingE<cugraph::test::Rmat_Usecase>;

TEST_P(Tests_MGPerVTopKSelectTransformOutgoingE_File, CheckInt32Int32FloatTupleIntFloat)
{
  auto param = GetParam();
...
TEST_P(Tests_MGPerVTopKSelectTransformOutgoingE_Rmat, CheckInt32Int32FloatTupleIntFloat)
{
  auto param = GetParam();
...
TEST_P(Tests_MGPerVTopKSelectTransformOutgoingE_Rmat, CheckInt64Int64FloatTupleIntFloat)
{
  auto param = GetParam();
...
TEST_P(Tests_MGPerVTopKSelectTransformOutgoingE_File, CheckInt32Int32Float)
{
  auto param = GetParam();
  run_current_test<int32_t, int32_t, float, int>(std::get<0>(param), std::get<1>(param));
}

TEST_P(Tests_MGPerVTopKSelectTransformOutgoingE_Rmat, CheckInt32Int32Float)
{
  auto param = GetParam();
...
TEST_P(Tests_MGPerVTopKSelectTransformOutgoingE_Rmat, CheckInt64Int64Float)
{
  auto param = GetParam();
...
INSTANTIATE_TEST_SUITE_P(
  file_test,
  Tests_MGPerVTopKSelectTransformOutgoingE_File,
  ::testing::Combine(
    ::testing::Values(Prims_Usecase{size_t{1000}, {4}, false, false, false, false},
                      Prims_Usecase{size_t{1000}, {4}, false, true, false, false},
                      Prims_Usecase{size_t{1000}, {4}, false, false, true, false},
                      Prims_Usecase{size_t{1000}, {4}, false, true, true, false},
                      Prims_Usecase{size_t{1000}, {4}, true, false, false, false},
                      Prims_Usecase{size_t{1000}, {4}, true, true, false, false},
                      Prims_Usecase{size_t{1000}, {4}, true, false, true, false},
                      Prims_Usecase{size_t{1000}, {4}, true, true, true, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, false, false, false, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, false, true, false, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, false, false, true, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, false, true, true, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, true, false, false, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, true, true, false, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, true, false, true, false},
                      Prims_Usecase{size_t{1000}, {2, 2, 0}, true, true, true, false}),
...
INSTANTIATE_TEST_SUITE_P(
  rmat_small_test,
  Tests_MGPerVTopKSelectTransformOutgoingE_Rmat,
  ::testing::Combine(
    ::testing::Values(Prims_Usecase{size_t{1000}, {4}, false, false, false, false},
                      Prims_Usecase{size_t{1000}, {4}, false, true, false, false},
                      Prims_Usecase{size_t{1000}, {4}, false, false, true, false},
...
INSTANTIATE_TEST_SUITE_P(
  ...
  Tests_MGPerVTopKSelectTransformOutgoingE_Rmat,
  ::testing::Combine(
    ::testing::Values(Prims_Usecase{size_t{10000000}, {25}, false, false, false, false},
                      Prims_Usecase{size_t{10000000}, {25}, false, true, false, false},
                      Prims_Usecase{size_t{10000000}, {25}, false, false, true, false},
                      Prims_Usecase{size_t{10000000}, {25}, false, true, true, false},
                      Prims_Usecase{size_t{10000000}, {25}, true, false, false, false},
                      Prims_Usecase{size_t{10000000}, {25}, true, true, false, false},
                      Prims_Usecase{size_t{10000000}, {25}, true, false, true, false},
                      Prims_Usecase{size_t{10000000}, {25}, true, true, true, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, false, false, false, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, false, true, false, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, false, false, true, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, false, true, true, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, true, false, false, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, true, true, false, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, true, false, true, false},
                      Prims_Usecase{size_t{10000000}, {10, 0, 15}, true, true, true, false}),
