...
__device__ uint32_t MurmurHash3_32_tail(uint8_t *data, int len_byte_tail, int len_total, uint32_t hash_begin = 32435354) {
...
__device__ uint32_t calc_murmur32_hash(int8_t *loc_1, int8_t *loc_2,
                                        int32_t len_str1, int32_t len_str2) {
  ...
  if (trailing_chars != 0) {
    hashed_str_val = MurmurHash3_32_tail(reinterpret_cast<uint8_t*>(&data), trailing_chars,
                                          len_str1+len_str2, hashed_str_val);
  }
  return hashed_str_val;
}

__device__ __inline__ bool compare_string(size_type src_idx, size_type match_idx,
                                          const int8_t *col_1, const int32_t *offset_1,
                                          const int8_t *col_2, const int32_t *offset_2) {
  // match length
  ...
  if ((length_a_0 == length_b_0) && (length_a_1 == length_b_1)) {
    // match first part
    while(length_a_0 > 0) {
      if (col_1[start_a_0++] != col_1[start_b_0++]) {
        return false;
      }
      length_a_0--;
    }
    ...
    // match second part
    while(length_a_1 > 0) {
      if (col_2[start_a_1++] != col_2[start_b_1++]) {
        return false;
      }
      length_a_1--;
    }
...
__device__ __inline__ bool compare_string_2(size_type src_idx, size_type match_idx,
                                          int8_t *base_col_1, int32_t *base_offset_1,
                                          int8_t *base_col_2, int32_t *base_offset_2,
                                          int8_t *col_1, int32_t *offset_1,
                                          int8_t *col_2, int32_t *offset_2) {
  // match length
  ...
  if ((length_a_0 == length_b_0) && (length_a_1 == length_b_1)) {
    // match first part
    while(length_a_0 > 0) {
      if (base_col_1[start_a_0++] != col_1[start_b_0++]) {
        return false;
      }
      length_a_0--;
    }
    ...
    // match second part
    while(length_a_1 > 0) {
      if (base_col_2[start_a_1++] != col_2[start_b_1++]) {
        return false;
      }
      length_a_1--;
    }
...
__global__ void concat_and_create_histogram(int8_t *col_1, int32_t *offset_1,
                                            int8_t *col_2, int32_t *offset_2,
                                            size_type num_rows, cudf_map_type hash_map,
                                            accum_type *sysmem_insert_counter) {

  extern __shared__ int8_t smem_[];
  int32_t *smem_col_1_offsets = reinterpret_cast<int32_t*>(smem_);
  int32_t *smem_col_2_offsets = reinterpret_cast<int32_t*>(smem_ + ((blockDim.x+1) * sizeof(int32_t)));
  accum_type *insert_counter = reinterpret_cast<uint32_t*>(smem_ + ((blockDim.x+1) * 2 * sizeof(int32_t)));

  int warp_accum_idx = threadIdx.x / warpSize;

  if ((threadIdx.x % warpSize) == 0)
    insert_counter[warp_accum_idx] = 0;
  __syncwarp();
  ...
  if (start_idx < num_rows) {
    ...
    auto insert_pair = hash_map.insert(thrust::make_pair(hashed_str_val, str_hash_value{start_idx, 0, 0}));
    ...
    if (!insert_pair.second) {
      ...
      while (!compare_string(row__, start_idx, col_1, offset_1, col_2, offset_2)) {
        ...
        insert_pair = hash_map.insert(thrust::make_pair(hashed_str_val, str_hash_value{start_idx, 0, 0}));
        if (insert_pair.second) {
...
__global__ void concat_and_create_histogram_2(int8_t *col_1, int32_t *offset_1,
                                              int8_t *col_2, int32_t *offset_2,
                                              int8_t *match_col_1, int32_t *match_offset_1,
                                              int8_t *match_col_2, int32_t *match_offset_2,
                                              size_type num_rows, cudf_map_type hash_map,
                                              accum_type *sysmem_insert_counter) {
  ...
  auto insert_pair = hash_map.insert(thrust::make_pair(hashed_str_val, str_hash_value{start_idx, 0, 1}));
  ...
  if (!insert_pair.second) {
    ...
    while (1) {
      if (col__ == 0) {
        if (compare_string_2(row__, start_idx, match_col_1, match_offset_1,
                              match_col_2, match_offset_2,
                              col_1, offset_1, col_2, offset_2))
          break;
      } else if (col__ == 1) {
        if (compare_string(row__, start_idx, col_1, offset_1, col_2, offset_2))
          break;
      }
      // else loop over +1 count of hash value and insert again
      hashed_str_val += hash_inc_constant;
      //printf("new insert\n");
      insert_pair = hash_map.insert(thrust::make_pair(hashed_str_val,
                                              str_hash_value{start_idx, 0, 1}));
      if (insert_pair.second) {
        atomicAdd(&(insert_counter[warp_accum_idx]), 1);
        break;
      }
...
template <typename T>
__global__ void set_src_vertex_idx(int8_t *col_1, int32_t *offset_1, int8_t *col_2, int32_t *offset_2,
                                    size_type num_rows, cudf_map_type lookup_table, T* out_vertex_mapping) {
  extern __shared__ int8_t smem_[];
  int32_t *smem_col_1_offsets = reinterpret_cast<int32_t*>(smem_);
  int32_t *smem_col_2_offsets = reinterpret_cast<int32_t*>(smem_ + ((blockDim.x+1) * sizeof(int32_t)));
...
template <typename T>
__global__ void set_dst_vertex_idx(int8_t *col_1, int32_t *offset_1, int8_t *col_2, int32_t *offset_2,
                                  int8_t *match_col_1, int32_t *match_offset_1, int8_t *match_col_2,
                                    int32_t *match_offset_2, size_type num_rows,
                                    cudf_map_type lookup_table, T* out_vertex_mapping) {
  extern __shared__ int8_t smem_[];
  int32_t *smem_col_1_offsets = reinterpret_cast<int32_t*>(smem_);
  int32_t *smem_col_2_offsets = reinterpret_cast<int32_t*>(smem_ + ((blockDim.x+1) * sizeof(int32_t)));
  ...
  if (start_idx < num_rows) {
    ...
    while (it != lookup_table.end()) {
      if (it->second.col_ == 0) {
        if (compare_string_2(it->second.row_, start_idx,
                        match_col_1, match_offset_1, match_col_2, match_offset_2,
                        col_1, offset_1, col_2, offset_2)) {
          out_vertex_mapping[start_idx] = (T)it->second.count_;
          break;
        }
...
__global__ void create_mapping_histogram(uint32_t *hash_value, str_hash_value *payload,
                                          cudf_map_type hash_map, accum_type count) {
  accum_type idx = threadIdx.x + blockIdx.x * blockDim.x;

  if (idx < count) {
    auto it = hash_map.insert(thrust::make_pair(hash_value[idx], payload[idx]));
  }
}

__global__ void assign_histogram_idx(cudf_map_type cuda_map_obj, size_t slot_count,
                                      str_hash_value *key, uint32_t *value,
                                      size_type *counter) {
...
__global__ void set_output_col_offsets(str_hash_value *row_col_pair, int32_t *out_col1_offset,
                                        int32_t *out_col2_offset, int dst_pair_match,
                                        int32_t *in_col1_offset, int32_t *in_col2_offset,
...
__global__ void offset_buffer_size_comp(int32_t *out_col1_length, int32_t *out_col2_length,
                                        int32_t *out_col1_offsets, int32_t *out_col2_offsets,
                                        accum_type total_elem, accum_type *out_sum) {

  int32_t idx = threadIdx.x + blockIdx.x * blockDim.x;

  if (idx == 0) {
    accum_type sum = accum_type(out_col1_offsets[total_elem - 1] +
                                  out_col1_length[total_elem - 1]);
    out_col1_offsets[total_elem] = sum;
    out_sum[0] = sum;
  } else if (idx == 1) {
    accum_type sum = accum_type(out_col2_offsets[total_elem - 1] +
                                  out_col2_length[total_elem - 1]);
    out_col2_offsets[total_elem] = sum;
    out_sum[1] = sum;
  }
}

__global__ void select_unrenumber_string(str_hash_value *idx_to_col_row, int32_t total_elements,
                                          int8_t *src_col1, int8_t *src_col2,
                                          int32_t *src_col1_offsets, int32_t *src_col2_offsets,
                                          int8_t *dst_col1, int8_t *dst_col2,
                                          int32_t *dst_col1_offsets, int32_t *dst_col2_offsets,
                                          int8_t *col1_out, int8_t *col2_out,
                                          int32_t *col1_out_offsets, int32_t *col2_out_offsets) {
...
struct renumber_functor {
  ...
  std::tuple<std::unique_ptr<cudf::column>,
    ...
    return std::make_tuple(std::unique_ptr<cudf::column>(new cudf::column(
                                                            ...
                                                            0,
                                                            rmm::device_buffer{0,cudaStream_t{0}})
                                                          ),
  ...
  std::tuple<std::unique_ptr<cudf::column>,
    ...
    for (int i = 0; i < src_view.num_columns(); i++) {
      ...
      src_vertex_chars_ptrs.push_back(const_cast<char_type*>(str_col_view.chars().data<char_type>()));
      src_vertex_offset_ptrs.push_back(const_cast<str_offset_type*>(
    ...
    for (int i = 0; i < dst_view.num_columns(); i++) {
      ...
      dst_vertex_chars_ptrs.push_back(const_cast<char_type*>(str_col_view.chars().data<char_type>()));
      dst_vertex_offset_ptrs.push_back(const_cast<str_offset_type*>(
                                            str_col_view.offsets().data<str_offset_type>()));
    }
    ...
    *hist_insert_counter = 0;
    ...
    auto cuda_map_obj = cudf_map_type::create(std::max(
                                                  static_cast<size_t>(
    ...
    assign_histogram_idx<<<grid, block, 0, exec_strm>>>(*cuda_map_obj, cuda_map_obj->capacity(),
                                                  sort_key.data(), sort_value.data(),
                                                  atomic_idx.data());
    ...
    set_output_col_offsets<<<grid, block, 0, exec_strm>>>(sort_key.data(), out_col1_length.data(),
                                                          out_col2_length.data(), 0,
                                                          src_vertex_offset_ptrs[0],
                                                          src_vertex_offset_ptrs[1],
                                                          key_value_count);

    set_output_col_offsets<<<grid, block, 0, exec_strm>>>(sort_key.data(), out_col1_length.data(),
                                                          out_col2_length.data(), 1,
                                                          dst_vertex_offset_ptrs[0],
                                                          dst_vertex_offset_ptrs[1],
                                                          key_value_count);
    ...
    cub::DeviceScan::ExclusiveSum(nullptr, tmp_storage_bytes, out_col1_length.data(),
                                    out_col1_offsets.data(), key_value_count, exec_strm);
    rmm::device_buffer tmp_storage(tmp_storage_bytes, exec_strm);
    cub::DeviceScan::ExclusiveSum(tmp_storage.data(), tmp_storage_bytes, out_col1_length.data(),
                                    out_col1_offsets.data(), key_value_count, exec_strm);
    cub::DeviceScan::ExclusiveSum(tmp_storage.data(), tmp_storage_bytes, out_col2_length.data(),
                                    out_col2_offsets.data(), key_value_count, exec_strm);
    ...
    offset_buffer_size_comp<<<1, 32, 0, exec_strm>>>(out_col1_length.data(), out_col2_length.data(),
                                            out_col1_offsets.data(), out_col2_offsets.data(),
                                            key_value_count, hist_insert_counter);
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks, select_unrenumber_string, block.x, 0);
    grid.x = num_multiprocessors * num_blocks;
    select_unrenumber_string<<<grid, block, 0, exec_strm>>>(sort_key.data(), key_value_count,
                                              src_vertex_chars_ptrs[0], src_vertex_chars_ptrs[1],
                                              src_vertex_offset_ptrs[0], src_vertex_offset_ptrs[1],
                                              dst_vertex_chars_ptrs[0], dst_vertex_chars_ptrs[1],
                                              dst_vertex_offset_ptrs[0], dst_vertex_offset_ptrs[1],
    ...
    auto offset_col_1 = std::unique_ptr<cudf::column>( new cudf::column(
                                                            cudf::data_type(cudf::type_id::INT32),
                                                            key_value_count+1,
                                                            std::move(out_col1_offsets.release())));

    auto str_col_1 = std::unique_ptr<cudf::column>( new cudf::column(
                                                            cudf::data_type(cudf::type_id::INT8),
                                                            hist_insert_counter[0],
                                                            std::move(unrenumber_col1_chars)) );

    renumber_table_vectors.push_back(cudf::make_strings_column(size_type(key_value_count),
                                                                std::move(offset_col_1),
                                                                std::move(str_col_1), 0,
                                                                rmm::device_buffer(size_type(0),
                                                                                    exec_strm)));

    auto offset_col_2 = std::unique_ptr<cudf::column>( new cudf::column(
                                                            cudf::data_type(cudf::type_id::INT32),
                                                            key_value_count+1,
                                                            std::move(out_col2_offsets.release())));

    auto str_col_2 = std::unique_ptr<cudf::column>( new cudf::column(
                                                            cudf::data_type(cudf::type_id::INT8),
    ...
    create_mapping_histogram<<<grid, block, 0, exec_strm>>>(sort_value.data(), sort_key.data(),
                                                            *cuda_map_obj_mapping, key_value_count);
    CHECK_CUDA(cudaStreamSynchronize(exec_strm));
    ...
    set_src_vertex_idx<<<grid, block, smem_size, exec_strm>>>(src_vertex_chars_ptrs[0],
                                                      src_vertex_offset_ptrs[0],
                                                      src_vertex_chars_ptrs[1],
                                                      src_vertex_offset_ptrs[1],
                                                      num_rows, *cuda_map_obj_mapping,
                                                      reinterpret_cast<Dtype*>(src_buffer.data()));
    CHECK_CUDA(cudaStreamSynchronize(exec_strm));
    set_dst_vertex_idx<<<grid, block, smem_size, exec_strm>>>(dst_vertex_chars_ptrs[0],
                                                          dst_vertex_offset_ptrs[0],
                                                          dst_vertex_chars_ptrs[1],
                                                          dst_vertex_offset_ptrs[1],
                                                          src_vertex_chars_ptrs[0],
    ...
    cols_vector.push_back(std::unique_ptr<cudf::column>(new cudf::column(
                                                            cudf::data_type(cudf::type_id::INT32),
                                                            num_rows,
                                                            std::move(src_buffer))
                                                          ));
