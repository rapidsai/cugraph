...
if [ -z ${CUGRAPH_ROOT:+PLACEHOLDER} ]; then
    CUGRAPH_BUILD_DIR=$(git rev-parse --show-toplevel 2>&1)/cpp/build
    status=$?
else
    CUGRAPH_BUILD_DIR=${CUGRAPH_ROOT}
fi
...
DEFAULT_FORMAT_FILE_LOCATIONS=(
  "${CUGRAPH_BUILD_DIR:-${HOME}}/_deps/rapids-cmake-src/cmake-format-rapids-cmake.json"
)
