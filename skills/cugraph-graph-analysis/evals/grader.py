...
def main():
    ...
    if case_id == "cugraph-analysis-explicit-directed-reachability":
        ...
        syntax = subprocess.run(["python3", "-m", "py_compile", str(source)], capture_output=True).returncode == 0 if source else False
        trusted = bool(reference and hashlib.sha256(reference.read_bytes()).hexdigest() == "6650d3af17a6b5bbbe1fd5d37b83d35675855d23b3732452bb0957ecdeee5be1")
        ref_ok = subprocess.run(["python3", str(reference)], capture_output=True).returncode == 0 if trusted else False
        checks = {
            ...
            "execution_evidence": (0.1, "cpu" in text and ("gpu" in text or "cugraph" in text)),
        }
        ...
        build_ok = bool(
            builds
            and {kw.arg for kw in builds[0].keywords} >= {"vertices", "renumber", "store_transposed"}
            and bool_keyword(builds[0], "renumber", True)
            and bool_keyword(builds[0], "store_transposed", True)
        )
        unweighted = bool(builds and not ({kw.arg for kw in builds[0].keywords} & {"edge_attr", "weight"}))
        tuple_unpack = any(
            isinstance(node, ast.Assign)
            and any(isinstance(target, (ast.Tuple, ast.List)) and len(target.elts) == 2 for target in node.targets)
        ...
        ).returncode == 0 if source else False
        checks = {
            ...
            "execution_evidence": (0.1, "gpu" in text and any(term in text for term in ("not", "block", "unavailable", "separate"))),
        }
        ...
        component_calls = ast_calls(tree, "weakly_connected_components") + ast_calls(tree, "strongly_connected_components")
        api_ok = len(component_calls) >= 2 and all(
            not ({kw.arg for kw in call.keywords} & {"directed", "connection", "return_labels"})
            for call in component_calls
        )
        partition_ok = ("canonical" in lowered or "frozenset" in lowered) and "duplicated" in lowered
        syntax_ok = subprocess.run(
            ["python3", "-m", "py_compile", str(source)], capture_output=True
        ).returncode == 0 if source else False
        checks = {
        ...
        checks = {
            "graph_vs_multigraph": (0.25, "simple graph" in text and "multigraph" in text),
            "aggregation_is_semantic": (0.25, "aggregate" in text and any(term in text for term in ("semantic", "meaning", "question", "choose"))),
            "algorithm_support": (0.25, "algorithm" in text and any(term in text for term in ("support", "compatible", "accept"))),
            "edge_reconciliation": (0.25, any(term in text for term in ("reconcil", "raw edge", "edge count", "transfer count", "number_of_edges", "len(raw)"))),
        }
    elif case_id == "cugraph-analysis-contextual-centrality":
        checks = {
            "distinct_meanings": (0.25, "in-degree" in text and "pagerank" in text and "betweenness" in text),
            "no_unexplained_average": (0.25, "average" in text and "not" in text),
            "parameters_and_convergence": (0.25, "alpha" in text and ("tolerance" in text or "tol" in text) and "converg" in text),
            "graph_scoped_interpretation": (0.25, "graph" in text and any(term in text for term in ("within", "definition", "scope", "does not"))),
        }
    elif case_id == "cugraph-analysis-implicit-composite-identities":
        checks = {
            "rejects_concatenation": (0.25, "concat" in text and any(term in text for term in ("collision", "not", "avoid", "do not"))),
            "column_lists": (0.25, any(term in text.replace(" ", "") for term in ("source=[", "source=src")) and any(term in text.replace(" ", "") for term in ("destination=[", "destination=dst"))),
            "renumbers": (0.25, "renumber=true" in text.replace(" ", "")),
            "keyed_handoff": (0.25, "0_vertex" in text and "1_vertex" in text and ("tenant" in text or "account" in text)),
        }
    elif case_id == "cugraph-analysis-contextual-symmetrization-metadata":
        checks = {
            "diagnoses_incompatibility": (0.25, "symmetr" in text and "edge_id" in text and "edge_type" in text and any(term in text for term in ("incompat", "cannot", "not supported", "fails"))),
            "separates_artifacts": (0.25, any(term in text for term in ("raw edge", "source edge", "typed edge", "raw relationship", "relationship_lineage", "audit_edges", "audit sidecar")) and "topology" in text),
            "reconciles_topology": (0.25, any(term in text for term in ("reconcil", "edge count", "distinct pair", "relationship count", "distinct-topology", "row counts"))),
            "no_invented_identity": (0.25, "reverse" in text and any(term in text for term in ("do not", "don't", "not invent", "policy"))),
        }
    else:
        checks = {
            "negative_routing": (0.7, "cudf" in text or "tabular" in text),
            "no_graph": (0.3, "do not" in text or "unnecessary" in text or "no graph" in text),
        }
    ...
    reward = {"overall": score, "custom_metrics": {"graph_contract_correctness": score}, "details": details}
