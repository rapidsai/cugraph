...
if [ "$BUILD_CUGRAPH" == "1" ]; then
  ...
  if [[ -z "$PROJECT_FLASH" || "$PROJECT_FLASH" == "0" ]]; then
    ...
    gpuci_conda_retry mambabuild --no-build-id --croot ${CONDA_BLD_DIR} conda/recipes/cugraph-pyg --python=$PYTHON -c pyg -c pytorch
    ...
    gpuci_logger "cugraph-pyg"
    gpuci_conda_retry mambabuild --no-build-id --croot ${CONDA_BLD_DIR} conda/recipes/cugraph-pyg -c ${CONDA_LOCAL_CHANNEL} --dirty --no-remove-work-dir --python=$PYTHON -c pyg -c pytorch
    gpuci_logger "cugraph-dgl"
