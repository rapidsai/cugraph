...
artifact_name=$(RAPIDS_PY_WHEEL_NAME="libraft_${RAPIDS_PY_CUDA_SUFFIX}" RAPIDS_REPOSITORY=raft PYTHON_VERSION="3.11" rapids-package-name wheel_python)
commit=$(git ls-remote https://github.com/rapidsai/raft.git refs/heads/pull-request/1512 | cut -c1-7)
libraft_wheelhouse=$(rapids-get-artifact "ci/raft/pull-request/1512/${commit}/${artifact_name}")
