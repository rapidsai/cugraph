...
./ci/run_cugraph_pytests.sh \
  ...
  --dist=worksteal \
  -m "not mg" \
  -k "not test_property_graph and not test_bulk_sampler and not test_create_undirected_graph_from_asymmetric_adj_list and not test_uniform_neighbor_sample and not test_node2vec" \
  --cov-config=../../.coveragerc \
  --cov=cugraph \
