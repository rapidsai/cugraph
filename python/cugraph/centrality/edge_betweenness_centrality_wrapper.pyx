...
def edge_betweenness_centrality(input_graph, normalized, weight, k,
    ...
    df['dst'] = indices.copy()#cudf.Series(np.zeros(number_of_edges, dtype=np.int32))
    ...
    if result_dtype == np.float32:
        graph_float = GraphCSRView[int, int, float](<int*> c_offsets, <int*> c_indices,
                                                <float*> NULL, number_of_vertices, number_of_edges)
        ...
        c_edge_betweenness_centrality[int, int, float, float](graph_float,
                                                         <float*> c_betweenness,
                                                         normalized,
                                                         <float*> c_weight, c_k,
                                                         <int*> c_vertices)
        # TODO(xcadet) How do we reconstruct the (src -> dst) association with
        # the EBC values?
        ...
        graph_double = GraphCSRView[int, int, double](<int*>c_offsets, <int*>c_indices,
                                                  <double*> NULL, number_of_vertices, number_of_edges)
        # fixme: there might be a way to avoid manually setting the graph property
        graph_double.prop.directed = type(input_graph) is DiGraph

        c_edge_betweenness_centrality[int, int, double, double](graph_double,
