...
def from_edgelist(
    ...
    if df_type is cudf.DataFrame:
        return from_cudf_edgelist(
            df, source, destination, edge_attr=edge_attr, create_using=create_using, renumber=renumber
        )
        ...
        G.from_dask_cudf_edgelist(df, source, destination, edge_attr=edge_attr, renumber=renumber)
        return G
