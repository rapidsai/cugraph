...
def test_types_from_numerals(dask_client):
    ...
    df_edgelist_cow = dask_cudf.from_cudf(cudf.DataFrame(
        {
            "src": [0, 7, 2, 0, 1],
            "dst": [1, 1, 1, 3, 2],
            "val": [1, 3, 2, 3, 3],
    ...
    df_edgelist_pig = dask_cudf.from_cudf(cudf.DataFrame(
        {
            "src": [3, 1, 4, 5, 6],
            "dst": [1, 6, 5, 6, 7],
            "val": [5, 4, 5, 5, 2],
    ...
    df_props_duck = dask_cudf.from_cudf(cudf.DataFrame(
        {
            "id": [0, 1, 2, 3],
            "a": [0, 1, 6, 2],
            "b": [2, 1, 2, 2],
    ...
    df_props_goose = dask_cudf.from_cudf(cudf.DataFrame(
        {
            "id": [4, 5, 6, 7],
            "a": [5, 4, 1, 8],
            "b": [2, 3, 8, 9],
