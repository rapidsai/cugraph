...
def test_create_mg_graph(dask_client, input_combo):
    ...
    start = dask_cudf.from_cudf(cudf.Series([1]), len(G._plc_graph))
    data_start = get_distributed_data(start)
    #assert len(G._plc_graph) == data_start.npartitions
    ...
    res = [
        ...
        for mg_graph, (w, st) in zip(G._plc_graph, data_start.worker_to_parts.items())
    ]

    wait(res)

    cudf_result = [dask_client.submit(convert_to_cudf,
                                 cp_arrays)
