...
@pytest.mark.parametrize("cugraph_input_type", utils.CUGRAPH_INPUT_TYPES)
def test_bfs_invalid_start(gpubenchmark, dataset_nxresults_startvertex_spc, cugraph_input_type):    
    (dataset, directed, nx_values, start_vertex, depth_limit) = dataset_nxresults_startvertex_spc
    
    # renumber the dataset so that start vertex is no longer a valid vertex
    el = cudf.read_csv(dataset, sep=' ', names=['src','tar','w'], dtype=['int32','int32','float32']).dropna()
    newval = max(el.src.max(), el.tar.max()) + 1
    el.src = el.src.replace(start_vertex, newval)
    el.tar = el.tar.replace(start_vertex, newval)
    G = cugraph.from_cudf_edgelist(el, source='src', destination='tar', edge_attr='w', renumber=True)
