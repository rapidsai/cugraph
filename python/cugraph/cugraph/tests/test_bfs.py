...
def get_nx_graph_and_params(dataset, directed):
    ...
    """
    return (dataset, directed, utils.generate_nx_graph_from_file(dataset, directed))
...
def single_dataset_nxresults_startvertex_spc(single_small_dataset_nx_graph, request):
    return get_nx_results_and_params(*request.param, *single_small_dataset_nx_graph)
...
def test_bfs_invalid_start(
    ...
    el = cudf.read_csv(
        dataset, sep=" ", names=["src", "tar", "w"], dtype=["int32", "int32", "float32"]
    ).dropna()
    ...
    G = cugraph.from_cudf_edgelist(
        el, source="src", destination="tar", edge_attr="w", renumber=True
    )
...
def test_scipy_api_compat():
    ...
    input_cugraph_graph = utils.create_obj_from_csv(
        graph_file, cugraph.Graph, edgevals=True
    )
    input_coo_matrix = utils.create_obj_from_csv(
        graph_file, cp_coo_matrix, edgevals=True
    )
