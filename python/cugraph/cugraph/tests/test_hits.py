...
def input_expected_output(input_combo):
    ...
    if "nxResults" not in input_combo:
        Gnx = utils.generate_nx_graph_from_file(
            input_combo["graph_file"], directed=True
        )
        nxResults = nx.hits(
...
def test_nx_hits(benchmark, input_combo):
    ...
    """
    Gnx = utils.generate_nx_graph_from_file(input_combo["graph_file"], directed=True)
    nxResults = benchmark(
...
def test_hits(benchmark, input_expected_output):
    G = utils.generate_cugraph_graph_from_file(input_expected_output["graph_file"])
