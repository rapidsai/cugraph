...
from cugraph.community.spectral_clustering cimport (
    balancedCutClustering as c_balanced_cut_clustering,
    spectralModularityMaximization as c_spectral_modularity_maximization,
    analyzeClustering_modularity as c_analyze_clustering_modularity,
    analyzeClustering_edge_cut as c_analyze_clustering_edge_cut,
    analyzeClustering_ratio_cut as c_analyze_clustering_ratio_cut,
)
...
def spectralBalancedCutClustering(input_graph,
    ...
    if isinstance(input_graph, cugraph.Graph):
        ...
        raise TypeError(
            f"only cugraph.Graph objects are supported, got: {type(input_graph)}"
        )
...
def spectralModularityMaximizationClustering(input_graph,
    ...
    if input_graph.adjlist.weights is None:
        raise Exception(
            "spectral modularity maximization must be called on a graph with weights"
        )
...
def analyzeClustering_modularity(input_graph, n_clusters, clustering):
    ...
    if input_graph.adjlist.weights is None:
        raise Exception(
            "analyze clustering modularity must be called on a graph with weights"
        )
    if input_graph.adjlist.weights is not None:
        [weights] = graph_primtypes_wrapper.datatype_cast(
            [input_graph.adjlist.weights], [np.float32, np.float64])
