...
def sparse_hungarian(input_graph, workers, epsilon):
    ...
    if weights.dtype == np.float32:
        g_float = GraphCOOView[int, int, float](
            <int*> c_src,
            <int*> c_dst,
            <float*> c_weights,
            num_verts,
        ...
        g_double = GraphCOOView[int, int, double](
            <int*> c_src,
            <int*> c_dst,
            <double*> c_weights,
            num_verts,
...
def dense_hungarian(costs, num_rows, num_columns, epsilon):
    ...
    if costs.dtype == np.float32:
        cost = c_dense_hungarian[int, float](
            handle_[0],
            <float*> c_costs,
            num_rows,
            num_columns,
        ...
        cost = c_dense_hungarian[int, double](
            handle_[0],
            <double*> c_costs,
            num_rows,
            num_columns,
        ...
        raise ValueError(f"unsupported type: {costs.dtype}")
