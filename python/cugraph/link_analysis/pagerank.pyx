...
def pagerank(input_graph, alpha=0.85, personalization=None, max_iter=100, tol=1.0e-5, nstart=None):
    ...
    if input_graph.transposedadjlist:
        [offsets, indices] = graph.datatype_cast([input_graph.transposedadjlist.offsets, input_graph.transposedadjlist.indices], [np.int32])
        [weights] = graph.datatype_cast([input_graph.transposedadjlist.weights], [np.float32, np.float64])
        graph.add_transposed_adj_list(graph, offsets, indices, weights)
    else:
        ...
        offsets, indices, values = graph.get_transposed_adj_list(graph)
