...
cimport cugraph.community.leiden as c_leiden
...
def leiden(input_graph, gamma=1.0, metric=0, max_level=100):
    ...
    cdef uintptr_t c_leiden_ptr = df['partition'].__cuda_array_interface__['data'][0]
    
    if g.adjList.edge_data.dtype == np.float32:
        c_leiden.leiden[int32_t, float](<Graph*>g, metric, gamma, <int32_t*>c_leiden_ptr, max_level)
    else:
        c_leiden.leiden[int32_t, double](<Graph*>g, metric, gamma, <int32_t*>c_leiden_ptr, max_level)
