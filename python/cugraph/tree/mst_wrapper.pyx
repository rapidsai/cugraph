...
from cugraph.tree.mst cimport mast as c_mst
...
def mst_float(input_graph):
    cdef GraphCOOViewFloat in_graph = get_graph_view[GraphCOOViewFloat](input_graph)
    return coo_to_df(move(mst[int,int,float](in_graph)))


def mst_double(input_graph):
...
def minimum_spanning_tree(input_graph):
    ...
    if graph_primtypes_wrapper.weight_type(input_graph) == np.float32:
        ...
        c_mst[int,int,float](handle_[0], graph_float)
        ...
        c_mst[int,int,double](handle_[0], graph_double)
