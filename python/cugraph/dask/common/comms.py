...
from cugraph.nccl import nccl
#from cugraph.dask.common.ucx import UCX

import weakref
from .comms_utils import func_build_comms
#from .comms_utils import inject_comms_on_handle, \
#    inject_comms_on_handle_coll_only, is_ucx_enabled
#from .utils import parse_host_port
#from cugraph.common.handle import Handle

from dask.distributed import get_worker, default_client

#from cugraph.utils.import_utils import has_ucp
import warnings
...
def worker_state(sessionId=None):
    ...
    """
    worker = get_worker()
    if not hasattr(worker, "_cugraph_comm_state"):
        worker._cugraph_comm_state = {}
    if sessionId is not None and sessionId not in worker._cugraph_comm_state:
        # Build state for new session and mark session creation time
        worker._cugraph_comm_state[sessionId] = {"ts": time.time()}

    if sessionId is not None:
        return worker._cugraph_comm_state[sessionId]
    return worker._cugraph_comm_state
...
async def _func_init_all(sessionId, uniqueId, comms_p2p,
    ...
    '''
    _func_build_comms(sessionId, verbose)
...
def _func_build_comms(sessionId, verbose):
    """
    Builds a cumlHandle on the current worker given the initialized comms
    :param nccl_comm: ncclComm_t Initialized NCCL comm
    :param nWorkers: int number of workers in cluster
    ...
    session_state["comm"] = func_build_comms(nccl_comm, nWorkers, workerId)
