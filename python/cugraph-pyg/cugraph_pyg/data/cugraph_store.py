...
class EXPERIMENTAL__CuGraphStore:
        ...
        *,
        ...
        if construct_graph:
            ...
            if self.__graph is None:
                self.__graph = self.__construct_graph(G, multi_gpu=multi_gpu, order=order)
    ...
    def __construct_graph(
        ...
        df = pandas.DataFrame(
            {
                "src": pandas.Series(na_dst) if order == 'CSC' else pandas.Series(na_src),
                "dst": pandas.Series(na_src) if order == 'CSC' else pandas.Series(na_dst),
        ...
        if multi_gpu:
            ...
            df = dd.from_pandas(df, npartitions=nworkers if len(df) > 32 else 1).persist()

            get_empty_df = lambda: cudf.DataFrame({'src':cudf.Series([], dtype=vertex_dtype), 'dst':cudf.Series([], dtype=vertex_dtype), 'etp': cudf.Series([], dtype='int32')})

            # Have to check for empty partitions and handle them appropriately
            df = df.map_partitions(
                lambda f : cudf.DataFrame.from_pandas(f) if len(f) > 0 else get_empty_df(),
                meta=get_empty_df(),
