...
def test_neighbor_sample(dask_client, rmm_global_pool, basic_graph_1):
    ...
    ).sort_values(by=['sources','destinations']).compute()
    ...
    assert row_dict[('vt1','pig','vt1')].tolist() == G[('vt1','pig','vt1')][0].tolist()
    assert col_dict[('vt1','pig','vt1')].tolist() == G[('vt1','pig','vt1')][1].tolist()
...
@pytest.mark.cugraph_ops
@pytest.mark.skipif(isinstance(torch, MissingModule), reason="torch not available")
def test_neighbor_sample_multi_vertex(dask_client, rmm_global_pool, multi_edge_multi_vertex_graph_1):
