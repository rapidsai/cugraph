...
cdef class SGGraph_From_CSR(_GPUGraph):
    ...
    def __cinit__(self,
        ...
        if not(isinstance(do_expensive_check, (int, bool))):
            raise TypeError("expected int or bool for do_expensive_check, got "
                            f"{type(do_expensive_check)}")
        assert_CAI_type(offset_array, "offset_array")
        assert_CAI_type(indice_array, "indice_array")
        ...
        # FIXME: assert that offset_array and indice_array have the same type
        ...
        cugraph_type_erased_device_array_view_free(offsets_view_ptr)
        cugraph_type_erased_device_array_view_free(indices_view_ptr)
        cugraph_type_erased_device_array_view_free(weights_view_ptr)
        if edge_id_view_ptr is not NULL:
            cugraph_type_erased_device_array_view_free(edge_id_view_ptr)
        if edge_type_view_ptr is not NULL:
