...
def create_temporal_graph(handle, n_vertices=100000, n_edges=1000000):
    ...
    edge_times = cp.array(np.sort(rng.integers(0, 365 * 24 * 3600, n_edges)), dtype=np.int64)
    
    graph_props = GraphProperties(is_symmetric=False, is_multigraph=False)
    graph = SGGraph(
        handle, graph_props, srcs, dsts,
...
def benchmark_standard(handle, graph, n_iterations=30, n_seeds=1000):
    ...
    for i in range(n_iterations):
        ...
        if i % 10 == 0:
            print(f"  Iter {i}: {elapsed:.2f} ms, {len(result.get('majors', []))} edges")
...
def main():
    ...
    graph, edge_times = create_temporal_graph(handle, n_vertices=100000, n_edges=1000000)
    ...
    _ = homogeneous_uniform_temporal_neighbor_sample(
        handle, graph, None, seeds, seed_times, None, fanout,
        with_replacement=True, do_expensive_check=False
    )
