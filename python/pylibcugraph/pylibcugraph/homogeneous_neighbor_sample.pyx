...
from pylibcugraph._cugraph_c.sampling_algorithms cimport (
    cugraph_homogeneous_neighbor_sample,
...
def heterogeneous_neighbor_sample(ResourceHandle resource_handle,
    ...
    h_fan_out: numpy array type
        ...
        starting vertex for each hop level. The sampling method uses the same
        fan_out value for each type.
    ...
    error_code = cugraph_homogeneous_neighbor_sample(
        ...
        &result_ptr,
        &error_ptr)
    assert_success(error_code, error_ptr, "cugraph_homogeneous_neighbor_sample")

    # Free the sampling options
    cugraph_sampling_options_free(sampling_options)

    if isinstance(h_fan_out, tuple):
        cugraph_heterogeneous_fan_out_free(fan_out_ptr)
