...
DATASETS_LARGE = [pytest.param(d) for d in utils.DATASETS_LARGE]
...
fixture_params_dts_lrg = utils.genFixtureParamsProduct(
    (DATASETS_LARGE, "grph"),
    (DIRECTED, "dirctd"),
    (WEIGHTED_GRAPH_OPTIONS, "wgtd_gph_opts"))
...
#Fixture added for the large dataset
@pytest.fixture(scope="module", params=fixture_params_dts_lrg)
def Fixture_params_dts_lrg(request):
    try:
        return utils.build_cu_and_nx_graphs(*request.param)
    except MemoryError:
        rmm.reinitialize(managed_memory=True)
...
#Test for the large dataset. omitting all parameters except G and Gnx -    
def test_betweenness_centrality_large(
    Fixture_params_dts_lrg
):
    prepare_test()
    sorted_df = calc_betweenness_centrality( #used default paramters of this function 
        Fixture_params_dts_lrg  
    )
    compare_scores(sorted_df, first_key="cu_bc", second_key="ref_bc")
